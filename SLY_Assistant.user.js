...
(async function () {
  ...
  class Assistant {
      ...
      if (this.enableAssistant === true) {
        ...
        while (waitForSequence) {
          ...
          for (let i = 0, n = userFleets.length; i < n; i++) {
            if (
              ["Mine Starting", "Mining Stop", "Unloading", "Loading"].includes(
                userFleets[i].state
              )
            )
              fleetBusy = true;
    ...
    async startAssistant() {
      for (let i = 0, n = userFleets.length; i < n; i++) {
        ...
        let fleetSavedData = await GM.getValue(
          userFleets[i].publicKey.toString(),
          "{}"
        );
  ...
  class BlockchainManager {
    //Token addresses
    programAddy = "ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL";
    tokenProgAddy = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA";

    //Commonly used public keys
    programPK = new solanaWeb3.PublicKey(this.programAddy);
    tokenProgramPK = new solanaWeb3.PublicKey(this.tokenProgAddy);

    //Program public keys
    /*
    sageProgramPK = new solanaWeb3.PublicKey('SAGEqqFewepDHH6hMDcmWy7yjHPpyKLDnRXKb3Ki8e6');
    profileProgramPK = new solanaWeb3.PublicKey('pprofELXjL5Kck7Jn5hCpwAL82DpTkSYBENzahVtbc9');
    cargoProgramPK = new solanaWeb3.PublicKey('Cargo8a1e6NkGyrjy4BQEW4ASGKs9KSyDyUrXMfpJoiH');
    profileFactionProgramPK = new solanaWeb3.PublicKey('pFACSRuobDmvfMKq1bAzwj27t6d2GJhSCHb1VcfnRmq');
      */
    sageProgramPK = new solanaWeb3.PublicKey(
      "SAGE2HAwep459SNq61LHvjxPk4pLPEJLoMETef7f7EE"
    );
    profileProgramPK = new solanaWeb3.PublicKey(
      "pprofELXjL5Kck7Jn5hCpwAL82DpTkSYBENzahVtbc9"
    );
    cargoProgramPK = new solanaWeb3.PublicKey(
      "Cargo2VNTPPTi9c1vq1Jw5d3BWUNr18MjRtSupAghKEk"
    );
    profileFactionProgramPK = new solanaWeb3.PublicKey(
      "pFACSRuobDmvfMKq1bAzwj27t6d2GJhSCHb1VcfnRmq"
    );
    craftingProgramPK = new solanaWeb3.PublicKey(
      "CRAFT2RPXPJWCEix4WpJST3E7NLf79GTqZUL75wngXo5"
    );
    pointsProgramId = new solanaWeb3.PublicKey(
      "Point2iBvz7j5TMVef8nEgpmz4pDr7tU7v3RjAfkQbM"
    );
    dataRunningXpCategory = new solanaWeb3.PublicKey(
      "DataJpxFgHhzwu4zYJeHCnAv21YqWtanEBphNxXBHdEY"
    );
    councilRankXpCategory = new solanaWeb3.PublicKey(
      "XPneyd1Wvoay3aAa24QiKyPjs8SUbZnGg5xvpKvTgN9"
    ); //??
    pilotingXpCategory = new solanaWeb3.PublicKey(
      "PiLotBQoUBUvKxMrrQbuR3qDhqgwLJctWsXj3uR7fGs"
    );
    miningXpCategory = new solanaWeb3.PublicKey(
      "MineMBxARiRdMh7s1wdStSK4Ns3YfnLjBfvF5ZCnzuw"
    );
    craftingXpCategory = new solanaWeb3.PublicKey(
    ...
    createPDA(derived, derivedFrom1, derivedFrom2, fleet) {
    ...
    getBalanceChange(txResult, targetAcct) {
      let acctIdx = txResult.transaction.message.staticAccountKeys.findIndex(
        (item) => item.toString() === targetAcct
      );
      let preBalanceObj = txResult.meta.preTokenBalances.find(
        (item) => item.accountIndex === acctIdx
      );
      let preBalance =
        preBalanceObj &&
        preBalanceObj.uiTokenAmount &&
        preBalanceObj.uiTokenAmount.uiAmount
          ? preBalanceObj.uiTokenAmount.uiAmount
          : 0;
      let postBalanceObj = txResult.meta.postTokenBalances.find(
        (item) => item.accountIndex === acctIdx
      );
      let postBalance =
        postBalanceObj &&
        postBalanceObj.uiTokenAmount &&
        postBalanceObj.uiTokenAmount.uiAmount
          ? postBalanceObj.uiTokenAmount.uiAmount
          : 0;
    ...
    async sendAndConfirmTx(
      ...
      if (curBlockHeight > lastValidBlockHeight)
        return {
          txHash,
      ...
      txHash = await this.solanaWriteConnection.sendRawTransaction(
        txSerialized,
        {
          skipPreflight: true,
          maxRetries: 0,
      ...
      while (curBlockHeight - interimBlockHeight < 10) {
        const signatureStatus =
          await this.solanaReadConnection.getSignatureStatus(txHash);
        if (
        ...
        await window.utils.timeUtils.wait(
    ...
    txSignAndSend(ix, fleet, opName, priorityFeeMultiplier) {
      return new Promise(async (resolve) => {
        ...
        while (!confirmed) {
          ...
          let latestBH = await this.solanaReadConnection.getLatestBlockhash(
            "confirmed"
          );
          ...
          if (typeof solflare === "undefined") {
            txSigned =
              phantom && phantom.solana
                ? await phantom.solana.signAllTransactions([tx])
                : solana.signAllTransactions([tx]);
          } else {
          ...
          let txResult = txHash
            ? await this.solanaReadConnection.getTransaction(txHash, {
                commitment: "confirmed",
                preflightCommitment: "confirmed",
          ...
          if (
            (confirmation.value &&
              ...
              confirmation.value.err.InstructionError) ||
            (txResult &&
              txResult.meta &&
              txResult.meta.err &&
              txResult.meta.err.InstructionError)
          ) {
            window.Fleet.prototype.updateFleetState(fleet, "ERROR: Ix Error");
            window.logger.cLog(
              2,
              `${window.utils.timeUtils.FleetTimeStamp(
                fleetName
            ...
            let ixError =
              txResult && txResult.meta && txResult.meta.logMessages
          ...
          if (!confirmation.name) {
            while (!txResult) {
              tryCount++;
              txResult = await this.solanaReadConnection.getTransaction(
                txHash,
                {
                  commitment: "confirmed",
                  preflightCommitment: "confirmed",
              ...
              if (!txResult) await window.utils.timeUtils.wait(1000);
            }
          }
  ...
  class Configuration {
    ...
    async saveTargetsImport() {
      ...
      for (let key in jsonTargets) {
        ...
        let scanBlock = window.proxyManager.buildScanBlock(destXStr, destYStr);
  ...
  class Fleet {
    ...
    calcNextWarpPoint(warpRange, startCoords, endCoords) {
      const [startX, startY] = [Number(startCoords[0]), Number(startCoords[1])];
      const [endX, endY] = [Number(endCoords[0]), Number(endCoords[1])];
      const moveDist = window.Movement.prototype.calculateMovementDistance(
        [startX, startY],
        [endX, endY]
      );
    ...
    calculateWarpTime(fleet, distance) {
      return fleet.warpSpeed > 0 ? distance / (fleet.warpSpeed / 1e6) : 0;
    }

    calcWarpFuelReq(fleet, startCoords, endCoords) {
      if (
        !window.utils.coordinateUtils.CoordsValid(startCoords) ||
        !window.utils.coordinateUtils.CoordsValid(endCoords)
      ) {
        window.logger.cLog(
          4,
          `${window.utils.timeUtils.FleetTimeStamp(
      ...
      while (!window.utils.coordinateUtils.CoordsEqual(curWP, endCoords)) {
        ...
        const distance = window.Movement.prototype.calculateMovementDistance(
          curWP,
          nextWP
        );
      ...
      //window.logger.cLog(4, `${window.utils.timeUtils.FleetTimeStamp(fleet.label)} calcWarpFuelReq: ${fuelRequired} fuel over ${jumps} jumps`);
      return fuelRequired;
    }

    calculateSubwarpTime(fleet, distance) {
      return fleet.subwarpSpeed > 0 ? distance / (fleet.subwarpSpeed / 1e6) : 0;
    }

    calculateSubwarpFuelBurn(fleet, distance) {
      return distance * (fleet.subwarpFuelConsumptionRate / 100);
    }

    getFleetState(fleetAcctInfo) {
      ...
      switch (remainingData[0]) {
        case 0:
          ...
          extra = sageProgram.coder.types.decode(
            "StarbaseLoadingBay",
            remainingData.subarray(1)
          );
          break;
        case 1: {
          fleetState = "Idle";
          let sector = sageProgram.coder.types.decode(
            "Idle",
            remainingData.subarray(1)
          );
    ...
    async execExitWarp(fleet) {
      return new Promise(async (resolve) => {
        let tx = {
          instruction: await sageProgram.methods
            ...
            .remainingAccounts([
              ...
              {
                pubkey: window.blockchainManager.pointsProgramId,
                isSigner: false,
                isWritable: false,
              },
            ])
        ...
        cLog(
          1,
          `${window.utils.timeUtils.FleetTimeStamp(fleet.label)} Exiting Warp`
        );
        this.updateFleetState(fleet, "Exiting Warp");

        let txResult = await window.blockchainManager.txSignAndSend(
          tx,
          fleet,
          "EXIT WARP"
        );
        ...
        let tx = {
          instruction: await sageProgram.methods
            ...
            .remainingAccounts([
              ...
              {
                pubkey: window.blockchainManager.cargoProgramPK,
                isSigner: false,
                isWritable: false,
              },
              {
    ...
    async execSubwarp(fleet, destX, destY, moveTime) {
      return new Promise(async (resolve) => {
        let tx = {
          instruction: await sageProgram.methods
            .startSubwarp({
              ...
              toSector: [
                new BrowserAnchor.anchor.BN(destX),
                new BrowserAnchor.anchor.BN(destY),
              ],
            })
            .accountsStrict({
        ...
        this.updateFleetState(fleet, "Subwarping");
        ...
        const travelEndTime = window.utils.timeUtils.TimeToStr(
          new Date(Date.now() + moveTime * 1000)
        );
        const newFleetState = `Subwarp ${coordStr} ${travelEndTime}`;
        this.updateFleetState(fleet, newFleetState);

        resolve(txResult);
      });
    }

    async execWarp(fleet, destX, destY, moveTime) {
      return new Promise(async (resolve) => {
        let fuelCargoTypeAcct = cargoTypes.find(
          (item) =>
            item.account.mint.toString() == sageGameAcct.account.mints.fuel
        ...
        fleet.warpCoolDownFinish =
          Date.now() + fleet.warpCooldown * 1000 + 2000;

        resolve({ txResult, warpCooldownFinished: fleet.warpCoolDownFinish });
      });
    }

    async execDock(fleet, dockCoords) {
      return new Promise(async (resolve) => {
        let starbaseX = dockCoords.split(",")[0].trim();
        let starbaseY = dockCoords.split(",")[1].trim();
        let starbase = await window.Starbase.prototype.getStarbaseFromCoords(
          starbaseX,
          starbaseY
        );
        ...
        starbasePlayer = starbasePlayer
          ...
          : await window.Starbase.prototype.execRegisterStarbasePlayer(
              fleet,
              dockCoords
            );
        let tx = {
    ...
    async execUndock(fleet, dockCoords) {
      return new Promise(async (resolve) => {
        ...
        let tx = {
          instruction: await sageProgram.methods
            .loadingBayToIdle(new BrowserAnchor.anchor.BN(userProfileKeyIdx))
            .accountsStrict({
              gameAccountsFleetAndOwner: {
                gameFleetAndOwner: {
                  fleetAndOwner: {
        ...
        this.updateFleetState(fleet, "Undocking");
        ...
        //await window.utils.timeUtils.wait(2000);
        this.updateFleetState(fleet, "Idle");
    ...
    async execStartupUndock(i, assignment) {
      ...
      if (assignment == "Transport" || assignment == "Mine") {
        ...
        const [fleetState, extra] = this.getFleetState(fleetAcctInfo);
        ...
        const fleetsCoords = [
          fleet.starbaseCoord.split(",")[0].trim(),
          fleet.starbaseCoord.split(",")[1].trim(),
        ];
    ...
    async fleetHealthCheck() {
        ...
        if (fleet.lastOp) {
          if (Date.now() - foo > 600000) {
            window.logger.cLog(
              ...
              foo ? window.utils.timeUtils.TimeToStr(new Date(foo)) : "null",
              fleet.lastOp
                ? window.utils.timeUtils.TimeToStr(new Date(fleet.lastOp))
                : "null",
              fleet.scanEnd
                ? window.utils.timeUtils.TimeToStr(new Date(fleet.scanEnd))
                : "null",
              fleet.moveEnd
                ? window.utils.timeUtils.TimeToStr(new Date(fleet.moveEnd))
                : "null"
            );
      ...
      if (window.assistant.enableAssistant)
        setTimeout(window.Fleet.prototype.fleetHealthCheck, 10000);
    }
    ...
    async fuelFleet(fleet, dockCoords, account, amount) {
      ...
      const fuelResp =
        await window.Starbase.prototype.execCargoFromStarbaseToFleet(
          fleet,
          fleet.fuelTank,
          account,
    ...
    async getAccountInfo(fleetName, reason, params) {
      ...
      //   params
      // );
      const returnValue = await window.blockchainManager.getAccountInfo(params);
      // return new SolanaAccountInfo(returnValue);
      return returnValue;
    }
    ...
    async getFleetFuelData(fleet, currentPos, targetPos) {
      ...
      const account = token
        ? token.pubkey
        : await this.getFleetFuelToken(fleet);
      const amount = token
        ? token.account.data.parsed.info.tokenAmount.uiAmount
        : 0;

      //window.logger.cLog(4, `${window.utils.timeUtils.FleetTimeStamp(fleet.label)} getFleetFuelData -> calcWarpFuelReq:`, currentPos, targetPos);
      return {
        account,
        token,
        amount,
        capacity: fleet.fuelCapacity,
        warpCost: this.calcWarpFuelReq(fleet, currentPos, targetPos),
        subwarpCost: Math.ceil(this.calculateSubwarpFuelBurn(fleet, moveDist)),
      };
    }

    async getFleetCntAtCoords() {
      ...
      if (!targetCoords || targetCoords.trim() === "") {
        loadingMessage.innerText =
          "Please enter target coordinates for grid center.";
      ...
      try {
        ...
        for (let i = 0; i < gridSize; i++) {
          ...
          for (let j = 0; j < gridSize; j++) {
            ...
            let xArr = xBN
              .toTwos(64)
              .toArrayLike(BrowserBuffer.Buffer.Buffer, "le", 8);
            ...
            let fleetAccts =
              await window.blockchainManager.solanaReadConnection.getProgramAccounts(
                window.blockchainManager.sageProgramPK,
                {
                  filters: [
            ...
            function calculateGradientColor(fleetCount) {
              ...
              const g = 0;
              const b = Math.floor(
                ((maxFleetCount - fleetCount) / maxFleetCount) * 255
              );
    ...
    async getFleetCargoToken(fleet, tokenPK) {
      const [token] =
        await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
          [
    ...
    async handleUndockAll() {
      for (let i = 0, n = userFleets.length; i < n; i++) {
        ...
        if (fleetState === "StarbaseLoadingBay") {
          let starbase = await sageProgram.account.starbase.fetch(
            extra.starbase
          );
          let coords =
            starbase.sector[0].toNumber() + "," + starbase.sector[1].toNumber();
    ...
    async handleTransportRefueling(
      fleet,
      starbaseCoord,
      currentPos,
      targetPos,
      ...
      if (fuelNeeded > fuelData.capacity) {
        window.logger.cLog(
          ...
          `${window.utils.timeUtils.FleetTimeStamp(
            fleet.label
          )} ERROR: Fuel tank too small for round trip`
        );
        fuelResp.detail = "ERROR: Fuel tank too small for round trip";
        return fuelResp;
      }

      const fuelEntry = transportManifest.find(
        (e) => e.res === sageGameAcct.account.mints.fuel.toString()
      ) || { amt: 0 };
      ...
      const fuelToAdd =
        Math.min(fuelData.capacity, totalFuel) - fuelData.amount;
    ...
    async handleTransportUnloading(fleet, starbaseCoord, transportManifest) {
      ...
      for (const entry of transportManifest) {
        if (entry.res !== "" && entry.amt > 0) {
          const isFuel =
            entry.res === sageGameAcct.account.mints.fuel.toString();
          const isAmmo = entry.res === ammoMint;
          const currentRes = fleetCurrentCargo.value.find(
            (item) => item.account.data.parsed.info.mint === entry.res
          );
          const currentResCnt = currentRes
            ? currentRes.account.data.parsed.info.tokenAmount.uiAmount
            : 0;
          ...
          if (amountToUnload > 0) {
            ...
            window.logger.cLog(
              1,
              `${window.utils.timeUtils.FleetTimeStamp(fleet.label)} Unload ${
                entry.res
      ...
      if (ammoEntry) {
        ...
        let currentAmmoCnt = currentAmmo
    ...
    async handleTransportLoading(i, starbaseCoords, transportManifest) {
      ...
      const ammoEntry = window.globalSettings.transportUseAmmoBank
        ...
        : undefined;
      let ammoLoadingIntoAmmoBank = ammoEntry
        ? await execLoadFleetAmmo(userFleets[i], starbaseCoords, ammoEntry.amt)
        : 0;
      ...
      const cargoCnt = fleetCurrentCargo.value.reduce(
        (n, { account }) =>
          ...
          account.data.parsed.info.tokenAmount.uiAmount *
            cargoItems.find((r) => r.token == account.data.parsed.info.mint)
              .size,
        0
      );
      ...
      window.logger.cLog(
        ...
        `${window.utils.timeUtils.FleetTimeStamp(
          userFleets[i].label
        )} cargoSpace remaining: ${cargoSpace}`
      );
      ...
      for (const entry of transportManifest) {
        if (entry.res && entry.amt > 0) {
          ...
          const fleetResAcct = currentRes
            ? currentRes.pubkey
            : fleetResourceToken;
          const resCargoTypeAcct = cargoTypes.find(
            (item) => item.account.mint.toString() == entry.res
          );

          //Deduct ammo already loaded into ammobank if applicable
          const isAmmo =
            entry.res === sageGameAcct.account.mints.ammo.toString();
          const resMax = Math.min(
            cargoSpace,
    ...
    async operateFleet(i) {
      ...
      const mining =
        userFleets[i].mineEnd &&
        userFleets[i].state.includes("Mine") &&
        Date.now() < userFleets[i].mineEnd;
      ...
      try {
        ...
        if (moving && fleetState == "Idle") {
          ...
          this.updateFleetState(userFleets[i], fleetState);
        }
        ...
        if (userFleets[i].iterCnt < 2 && fleetState == "StarbaseLoadingBay") {
          if (
            ...
            await window.Fleet.prototype.execStartupUndock(
              i,
          ...
          await window.Movement.prototype.handleMovement(i, null, null, null);
        } else if (
          fleetParsedData.assignment == "Scan" &&
          fleetState == "Idle"
        ) {
          this.updateFleetState(userFleets[i], fleetState);
          window.Scanning.prototype.startupScanBlockCheck(i, fleetCoords);
          const curentSBI = userFleets[i].scanBlockIdx;
          await window.Scanning.prototype.handleScan(
            i,
            fleetCoords,
            userFleets[i].scanBlock[curentSBI]
          );
          ...
          if (
            ...
            this.updateFleetState(
              userFleets[i],
              "Mine [" +
                window.utils.timeUtils.TimeToStr(new Date(Date.now())) +
                "]"
            );
          }
          await window.Mining.prototype.handleMining(
            i,
            userFleets[i].state,
            fleetCoords,
            fleetMining
          ...
          await window.game.handleTransport(
            i,
            userFleets[i].state,
            fleetCoords
          );
        ...
        window.logger.cLog(
          1,
          `${window.utils.timeUtils.FleetTimeStamp(userFleets[i].label)} ERROR`,
          err
        );
      }
    }
    ...
    async startFleet(i) {
      ...
      try {
        //window.logger.cLog(1,`${window.utils.timeUtils.FleetTimeStamp(userFleets[i].label)} Operating fleet ...`);
        const fleetSavedData = await GM.getValue(
          fleet.publicKey.toString(),
          "{}"
        );
        ...
        if (fleetParsedData.assignment) {
          ...
          if (
            ...
            await window.Scanning.prototype.createScannerPDAs(fleet);
  ...
  class Game {
    ...
    fleets = [];
    constructor() {
      let observer = new MutationObserver(this.waitForLabs);

      observer.observe(document, { childList: true, subtree: true });
      this.waitForLabs(null, null);
    }

    async getResourceTokens() {
      ...
      for (let resource of resources) {
        let cargoType = cargoTypes.find(
          (item) =>
            item.account.mint.toString() === resource.account.mint.toString()
        );
        let cargoName = new TextDecoder()
          .decode(new Uint8Array(resource.account.name))
          .replace(/\0/g, "");
        let cargoSize = await this.getCargoTypeSize(cargoType);
        cargoItems.push({
          name: cargoName,
          token: resource.account.mint.toString(),
          size: cargoSize,
        });
      }
      for (let craftable of craftables) {
        let cargoType = cargoTypes.find(
          (item) =>
            item.account.mint.toString() === craftable.account.mint.toString()
        );
        let cargoName = new TextDecoder()
          .decode(new Uint8Array(craftable.account.namespace))
          .replace(/\0/g, "");
        let cargoSize = await this.getCargoTypeSize(cargoType);
        cargoItems.push({
          name: cargoName,
          token: craftable.account.mint.toString(),
          size: cargoSize,
        });
      }
    ...
    async initUser() {
      return new Promise(async (resolve) => {
        ...
        if (
          ...
          let userProfiles =
            await window.blockchainManager.solanaReadConnection.getProgramAccounts(
              window.blockchainManager.profileProgramPK
            );
          let foundProf = [];

          window.logger.cLog(2, "initUser: userProfiles[0]", userProfiles[0]);
          for (let userProf of userProfiles) {
            ...
            while (userProfData.length >= 80) {
              ...
              let profDecoded = profileProgram.coder.types.decode(
                "ProfileKey",
                currProf
              );
              if (profDecoded.key.toString() === userPublicKey.toString()) {
                ...
                let playerName = playerNameAcct
                  ? new TextDecoder().decode(
                      playerNameAcct.account.data.subarray(42)
                    )
                  : "";
                foundProf.push({
          ...
          let userProfile =
            foundProf.length > 1
              ? await assistProfileToggle(foundProf)
              : foundProf[0];
        ...
        let profileFactionProgram = new BrowserAnchor.anchor.Program(
          profileFactionIDL,
          window.blockchainManager.profileFactionProgramPK,
          anchorProvider
        );
        [userProfileFactionAcct] =
          await profileFactionProgram.account.profileFactionAccount.all([
            {
              memcmp: {
                offset: 9,
        ...
        window.User.prototype.buildXpAccounts(
          window.blockchainManager.dataRunningXpCategory,
          userXpAccounts,
          "userDataRunningXpAccounts"
        );
        window.User.prototype.buildXpAccounts(
          window.blockchainManager.councilRankXpCategory,
          userXpAccounts,
          "userCouncilRankXpAccounts"
        );
        window.User.prototype.buildXpAccounts(
          window.blockchainManager.pilotingXpCategory,
          userXpAccounts,
          "userPilotingXpAccounts"
        );
        window.User.prototype.buildXpAccounts(
          window.blockchainManager.miningXpCategory,
          userXpAccounts,
        ...
        window.logger.cLog(1, "initUser: userFleetAccts", userFleetAccts);

        for (let fleet of userFleetAccts) {
          let fleetLabel = new TextDecoder("utf-8")
            .decode(new Uint8Array(fleet.account.fleetLabel))
            .replace(/\0/g, "");
          ...
          let fleetParsedData = JSON.parse(fleetSavedData);
          let fleetDest =
            fleetParsedData && fleetParsedData.dest ? fleetParsedData.dest : "";
          let fleetScanBlock =
            fleetParsedData && fleetParsedData.scanBlock
              ? fleetParsedData.scanBlock
              : [];
          let fleetScanMin =
            fleetParsedData && fleetParsedData.scanMin
              ? fleetParsedData.scanMin
              : 10;
          let fleetScanMove =
            (fleetParsedData && fleetParsedData.scanMove == "false") || false
              ? false
              : true;
          let fleetMineResource =
            fleetParsedData && fleetParsedData.mineResource
              ? fleetParsedData.mineResource
              : "";
          let fleetStarbase =
            fleetParsedData && fleetParsedData.starbase
              ? fleetParsedData.starbase
              : "";
          let fleetMoveType =
            fleetParsedData && fleetParsedData.moveType
              ? fleetParsedData.moveType
              : "warp";
          let fleetMoveTarget =
            fleetParsedData && fleetParsedData.moveTarget
              ? fleetParsedData.moveTarget
              : "";

          const [fleetRepairKitToken] =
            await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
              [
          ...
          let fleetCurrentCargo =
            await window.blockchainManager.solanaReadConnection.getParsedTokenAccountsByOwner(
              fleet.account.cargoHold,
              { programId: window.blockchainManager.tokenProgramPK }
            );
          let currentFoodCnt = fleetCurrentCargo.value.find(
            (item) => item.pubkey.toString() === fleetFoodToken.toString()
          );
          let currentSduCnt = fleetCurrentCargo.value.find(
            (item) => item.pubkey.toString() === fleetSduToken.toString()
          );
          let fleetCurrentFuel =
            await window.blockchainManager.solanaReadConnection.getParsedTokenAccountsByOwner(
              fleet.account.fuelTank,
              { programId: window.blockchainManager.tokenProgramPK }
            );
          let currentFuelCnt = fleetCurrentFuel.value.find(
            (item) => item.pubkey.toString() === fleetFuelToken.toString()
          );
          let fleetAcctInfo = await window.assistant.getAccountInfo(
            fleetLabel,
            "Full Account Info",
          ...
          userFleets.push({
            ...
            warpFuelConsumptionRate:
              fleet.account.stats.movementStats.warpFuelConsumptionRate,
            ...
            subwarpFuelConsumptionRate:
              fleet.account.stats.movementStats.subwarpFuelConsumptionRate,
            ...
            foodConsumptionRate:
              fleet.account.stats.cargoStats.foodConsumptionRate,
            ammoConsumptionRate:
              fleet.account.stats.cargoStats.ammoConsumptionRate,
            planetExitFuelAmount:
              fleet.account.stats.movementStats.planetExitFuelAmount,
            destCoord: fleetDest,
            ...
            foodCnt: currentFoodCnt
              ? currentFoodCnt.account.data.parsed.info.tokenAmount.uiAmount
              : 0,
            sduCnt: currentSduCnt
              ? currentSduCnt.account.data.parsed.info.tokenAmount.uiAmount
              : 0,
            fuelCnt: currentFuelCnt
              ? currentFuelCnt.account.data.parsed.info.tokenAmount.uiAmount
              : 0,
    ...
    waitForLabs(mutations, observer) {
      let elemTrigger = observer
    ...
    async getCargoTypeSize(cargoType) {
      let cargoTypeAcct =
        await window.blockchainManager.solanaReadConnection.getAccountInfo(
          cargoType.publicKey
        );
      let cargoTypeDataExtra = cargoTypeAcct.data.subarray(110);
      let cargoTypeDataExtraBuff =
        BrowserBuffer.Buffer.Buffer.from(cargoTypeDataExtra);
      let cargoTypeSize = cargoTypeDataExtraBuff.readUIntLE(0, 8);
      return cargoTypeSize;
    }

    async execCreateCargoPod(fleet, dockCoords) {
      let starbaseX = dockCoords.split(",")[0].trim();
      let starbaseY = dockCoords.split(",")[1].trim();
      let starbase = await window.Starbase.prototype.getStarbaseFromCoords(
        starbaseX,
      ...
      let cargoPodData = {
        keyIndex: new BrowserAnchor.anchor.BN(userProfileKeyIdx),
        podSeeds: Array.from(
          solanaWeb3.Keypair.generate().publicKey.toBuffer()
        ),
      };
      ...
      await window.blockchainManager.txSignAndSend(
        tx,
        fleet,
        "Create CargoPod"
      );
    ...
    async execCargoFromFleetToStarbase(
      fleet,
      fleetCargoPod,
      tokenMint,
      dockCoords,
      ...
      return new Promise(async (resolve) => {
        ...
        let starbasePlayerCargoHold = starbasePlayerCargoHolds.find(
          (item) => item.account.openTokenAccounts > 0
        );
        starbasePlayerCargoHold = starbasePlayerCargoHold
          ? starbasePlayerCargoHold.publicKey
          : starbasePlayerCargoHolds.length > 0
          ? starbasePlayerCargoHolds[0].publicKey
          : await execCreateCargoPod(fleet, dockCoords);
        ...
        let fleetResourceAcct = currentResource
          ? currentResource.pubkey
          : fleetResourceToken;
        let resourceCargoTypeAcct = cargoTypes.find(
          (item) => item.account.mint.toString() == tokenMint
        );
    ...
    async execLoadFleetAmmo(fleet, starbaseCoords, amount) {
      ...
      const parsedTokenAccount = parsedTokenAccounts.value.find(
        (item) => item.account.data.parsed.info.mint === ammoMint.toString()
      );
      const ammoCargoTypeAcct = cargoTypes.find(
        (item) => item.account.mint.toString() == ammoMint
      );
      const [fleetAmmoToken] =
        await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
          [
            fleet.ammoBank.toBuffer(),
            window.blockchainManager.tokenProgramPK.toBuffer(),
    ...
    async handleResupply(i, fleetCoords) {
      ...
      async function loadFood() {
        ...
        let foodCargoTypeAcct = cargoTypes.find(
          (item) =>
            item.account.mint.toString() == sageGameAcct.account.mints.food
        );
        ...
        //Update food count
        const foodCargo =
          await window.blockchainManager.solanaReadConnection.getTokenAccountBalance(
            userFleets[i].foodToken
          );
        ...
        if (userFleets[i].foodCnt < userFleets[i].scanCost) {
          ...
          window.Fleet.prototype.updateFleetState(
            userFleets[i],
            `ERROR: No food ⌛ ${window.utils.timeUtils.TimeToStr(
              new Date(Date.now() + errorWaitTime)
            )}`
          );
      ...
      async function loadFuel() {
        ...
        let currentFuelCnt = fleetCurrentFuel.value.find(
          (item) =>
            item.pubkey.toString() === userFleets[i].fuelToken.toString()
        );
        ...
        if (
          ...
          window.Fleet.prototype.updateFleetState(
            userFleets[i],
            `ERROR: No Fuel ⌛ ${window.utils.timeUtils.TimeToStr(
              new Date(Date.now() + errorWaitTime)
            )}`
          );
      ...
      const baseCoords = window.utils.coordinateUtils.ConvertCoords(
        userFleets[i].starbaseCoord
      );
      ...
      if (window.utils.coordinateUtils.CoordsEqual(fleetCoords, baseCoords)) {
        ...
        await window.Fleet.prototype.execDock(
          userFleets[i],
          userFleets[i].starbaseCoord
        );
        ...
        let moveDist = window.Movement.prototype.calculateMovementDistance(
          fleetCoords,
          baseCoords
        );
        if (moveDist > 0 && !userFleets[i].state.includes("Warp C/D")) {
          ...
          window.Fleet.prototype.updateFleetState(userFleets[i], "Idle");
          await window.Movement.prototype.handleMovement(
            i,
            moveDist,
            baseCoords[0],
            baseCoords[1]
  ...
  class Mining {
    calculateMiningDuration(
      ...
      return resourceHardness > 0
        ? Math.ceil(
            cargoCapacity /
              (((miningRate / 10000) * (systemRichness / 100)) /
                (resourceHardness / 100))
          )
        : 0;
    }
    async calcMiningFleet() {
      // EXPERIMENTAL
      console.log("userFleets: ", userFleets);
      for (let userFleet of userFleets) {
        console.log("-----", userFleet.label, "-----");
        let fleetSavedData = await GM.getValue(
          userFleet.publicKey.toString(),
          "{}"
        );
        let fleetParsedData = JSON.parse(fleetSavedData);
        ...
        if (fleetParsedData.assignment == "Mine") {
          ...
          let planets = await window.Starbase.prototype.getPlanetsFromCoords(
            destX,
            destY
          );
          ...
          if (sageResource && sageResource.account) {
            ...
            let resShort = cargoItems.find(
              (r) => r.token == userFleet.mineResource
            ).name;
            console.log(
          ...
          let miningDuration = window.Mining.prototype.calculateMiningDuration(
            userFleet.cargoCapacity,
            userFleet.miningRate,
            resourceHardness,
            systemRichness
          );
          let foodForDuration = Math.max(
            Math.ceil(
              (miningDuration - 10) * (userFleet.foodConsumptionRate / 10000)
            ),
            0
          );
          let ammoForDuration = Math.ceil(
            miningDuration * (userFleet.ammoConsumptionRate / 10000)
          );
          ammoForDuration = Math.min(userFleet.ammoCapacity, ammoForDuration);

          let distToTarget =
            window.Movement.prototype.calculateMovementDistance(
              [starbaseX, starbaseY],
              [destX, destY]
            );
          let distReturn = window.Movement.prototype.calculateMovementDistance(
            [destX, destY],
            [starbaseX, starbaseY]
          );
          let fuelNeeded = userFleet.planetExitFuelAmount;
          let warpCost =
            window.Fleet.prototype.calcWarpFuelReq(
              userFleet,
              [starbaseX, starbaseY],
              [destX, destY]
            ) *
          ...
          let halfWarpCost =
            ...
            window.Fleet.prototype.calculateSubwarpFuelBurn(
              userFleet,
              distReturn
            ) +
            userFleet.planetExitFuelAmount;
          ...
          const marketProgramId = new solanaWeb3.PublicKey(
            "traderDnaR5w6Tcoi3NFm53i48FTDNbGjBSZwWXDRrg"
          );
          const marketIDL = await BrowserAnchor.anchor.Program.fetchIdl(
            marketProgramId,
            anchorProvider
          );
          const marketProgram = new BrowserAnchor.anchor.Program(
            marketIDL,
            marketProgramId,
            anchorProvider
          );
          ...
          let fuelSellOrders = orderAccounts.filter((i) => {
            return (
              i.account.assetMint.toString() ==
                "fueL3hBZjLLLJHiFH9cqZoozTG3XQZ53diwFPwbzNim" &&
              Object.keys(i.account.orderSide)[0] == "sell"
            );
          });
          let fuelMinSellOrder = fuelSellOrders.reduce((a, b) =>
            a.account.price.toNumber() < b.account.price.toNumber() ? a : b
          );
          let fuelMinSellPrice =
            fuelMinSellOrder.account.price.toNumber() / Math.pow(10, 8);
          ...
          let ammoMinSellOrder = ammoSellOrders.reduce((a, b) =>
            a.account.price.toNumber() < b.account.price.toNumber() ? a : b
          );
          let ammoMinSellPrice =
            ammoMinSellOrder.account.price.toNumber() / Math.pow(10, 8);
          ...
          let foodMinSellOrder = foodSellOrders.reduce((a, b) =>
            a.account.price.toNumber() < b.account.price.toNumber() ? a : b
          );
          let foodMinSellPrice =
            foodMinSellOrder.account.price.toNumber() / Math.pow(10, 8);

          let resSellOrders = orderAccounts.filter((i) => {
            return (
              i.account.assetMint.toString() == userFleet.mineResource &&
              Object.keys(i.account.orderSide)[0] == "sell"
            );
          });
          let resMinSellOrder = resSellOrders.reduce((a, b) =>
            a.account.price.toNumber() < b.account.price.toNumber() ? a : b
          );
          let resMinSellPrice =
            resMinSellOrder.account.price.toNumber() / Math.pow(10, 8);
          ...
          console.log(
            "Sell: ",
            userFleet.cargoCapacity * resMinSellPrice,
            " ATLAS"
          );
          console.log(
            "Ammo: ",
            ammoForDuration,
            "[",
            ammoForDuration * ammoMinSellPrice,
            " ATLAS]"
          );
          console.log(
            "Food: ",
            foodForDuration,
            "[",
            foodForDuration * foodMinSellPrice,
            " ATLAS]"
          );
          console.log(
            "Fuel: ",
    ...
    async execStartMining(fleet, mineItem, sageResource, planet) {
      return new Promise(async (resolve) => {
        ...
        window.Fleet.prototype.updateFleetState(fleet, "Mine Starting");
    ...
    async execStopMining(
      ...
      return new Promise(async (resolve) => {
        ...
        const fleetAmmoToken = await window.Fleet.prototype.getFleetAmmoToken(
          fleet
        );
        const fleetCurrentCargo =
          await window.blockchainManager.solanaReadConnection.getParsedTokenAccountsByOwner(
        ...
        const currentFood = fleetCurrentCargo.value.find(
          (item) =>
            item.account.data.parsed.info.mint ===
            sageGameAcct.account.mints.food.toString()
        );
        ...
        let currentAmmo = fleetCurrentAmmoBank.value.find(
          (item) =>
            item.account.data.parsed.info.mint ===
            sageGameAcct.account.mints.ammo.toString()
        );
        ...
        let ammoCargoTypeAcct = cargoTypes.find(
          (item) =>
            item.account.mint.toString() == sageGameAcct.account.mints.ammo
        );
        let resourceCargoTypeAcct = cargoTypes.find(
          (item) => item.account.mint.toString() == resourceToken.toString()
        );
        let tx1 = {
          instruction: await sageProgram.methods
            .fleetStateHandler()
        ...
        let tx2 = {
          instruction: await sageProgram.methods
            ...
            .accountsStrict({
              ...
              tokenProgram:
                window.blockchainManager.window.blockchainManager
                  .tokenProgramPK,
            })
            .instruction(),
        };
    ...
    async handleMining(i, fleetState, fleetCoords, fleetMining) {
      ...
      if (sageResource && sageResource.account) {
        ...
        window.logger.cLog(
          ...
          `${window.utils.timeUtils.FleetTimeStamp(
            userFleets[i].label
          )} ERROR: ${resShort} not found at mining location`
        );
        window.Fleet.prototype.updateFleetState(
          userFleets[i],
          `ERROR: ${resShort} not found at mining location`
      ...
      let currentFuelCnt = currentFuel
        ? currentFuel.account.data.parsed.info.tokenAmount.uiAmount
        : 0;
      let fleetCurrentCargo =
        await window.blockchainManager.solanaReadConnection.getParsedTokenAccountsByOwner(
          userFleets[i].cargoHold,
      ...
      let currentFood = fleetCurrentCargo.value.find(
        (item) =>
          item.account.data.parsed.info.mint ===
          sageGameAcct.account.mints.food.toString()
      );
      let fleetFoodAcct = currentFood ? currentFood.pubkey : fleetFoodToken;
      let currentFoodCnt = currentFood
        ? currentFood.account.data.parsed.info.tokenAmount.uiAmount
        : 0;
      let currentResource = fleetCurrentCargo.value.find(
        (item) =>
          item.account.data.parsed.info.mint === userFleets[i].mineResource
      );
      let fleetResourceAcct = currentResource
        ? currentResource.pubkey
        : fleetResourceToken;
      let currentResourceCnt = currentResource
        ? currentResource.account.data.parsed.info.tokenAmount.uiAmount
        : 0;
      let fleetCurrentAmmoBank =
        await window.blockchainManager.solanaReadConnection.getParsedTokenAccountsByOwner(
      ...
      let foodForDuration = Math.ceil(
        miningDuration * (userFleets[i].foodConsumptionRate / 10000)
      );
      let ammoForDuration = Math.ceil(
        miningDuration * (userFleets[i].ammoConsumptionRate / 10000)
      );
      ammoForDuration = Math.min(userFleets[i].ammoCapacity, ammoForDuration);

      let distToTarget = window.Movement.prototype.calculateMovementDistance(
        fleetCoords,
        [destX, destY]
      );
      let distReturn = window.Movement.prototype.calculateMovementDistance(
      ...
      window.logger.cLog(
        ...
        `${window.utils.timeUtils.FleetTimeStamp(
          userFleets[i].label
        )} window.Mining.prototype.handleMining -> fleet:`,
        fleetCoords,
        `starbase:`,
        [starbaseX, starbaseY],
        `target:`,
      ...
      const warpCostToTarget =
        fleetCoords.length == 2
          ? window.Fleet.prototype.calcWarpFuelReq(userFleets[i], fleetCoords, [
              destX,
              destY,
            ])
          : 0;
      ...
      if (userFleets[i].moveType == "warp") {
        fuelNeeded +=
          userFleets[i].fuelCapacity < warpCost
            ? userFleets[i].fuelCapacity < halfWarpCost
              ? subwarpCost
              : halfWarpCost
            : warpCost;
      ...
      if (fleetState === "Idle") {
        ...
        if (miningDuration < 60) {
          ...
          currentFuelCnt < fuelNeeded ||
          currentAmmoCnt < ammoForDuration ||
          currentFoodCnt < foodForDuration
        ) {
          needSupplies = true;
        }
        ...
        if (needSupplies) {
          ...
          foodForDuration = Math.ceil(
            miningDuration * (userFleets[i].foodConsumptionRate / 10000)
          );
          ammoForDuration = Math.ceil(
            miningDuration * (userFleets[i].ammoConsumptionRate / 10000)
          );
          ammoForDuration = Math.min(
            userFleets[i].ammoCapacity,
            ammoForDuration
          );
          ...
          //window.logger.cLog(2, `${window.utils.timeUtils.FleetTimeStamp(userFleets[i].label)} ammoForDuration: ${ammoForDuration} = miningDuration ${miningDuration} * (ammoConsumptionRate ${userFleets[i].ammoConsumptionRate} / 10000)`);
          window.logger.cLog(
            2,
            `${window.utils.timeUtils.FleetTimeStamp(
              userFleets[i].label
            )} food: ${currentFoodCnt}/${foodForDuration}`
          );
          ...
          if (fleetCoords[0] == starbaseX && fleetCoords[1] == starbaseY) {
            ...
            if (currentFuelCnt < fuelNeeded) {
              ...
              window.logger.cLog(
                ...
                `${window.utils.timeUtils.FleetTimeStamp(
                  userFleets[i].label
                )} Fuel loading skipped: ${currentFuelCnt} / ${fuelNeeded}`
              );
            }
            ...
            if (currentAmmoCnt < ammoForDuration) {
              ...
              window.logger.cLog(
                ...
                `${window.utils.timeUtils.FleetTimeStamp(
                  userFleets[i].label
                )} Ammo loading skipped: ${currentAmmoCnt} / ${ammoForDuration}`
              );
            }
            ...
            if (currentFoodCnt < foodForDuration) {
              ...
              window.logger.cLog(
                ...
                `${window.utils.timeUtils.FleetTimeStamp(
                  userFleets[i].label
                )} Food loading skipped: ${currentFoodCnt} / ${foodForDuration}`
              );
            }
            ...
            await this.handleMineMovement(i, fleetCoords);
          }
        }

        //At mining area?
        else if (fleetCoords[0] == destX && fleetCoords[1] == destY) {
          await this.execStartMining(
            userFleets[i],
            mineItem,
            sageResource,
            planet
          ...
          if (userFleets[i].state.slice(0, 5) !== "ERROR")
            window.Fleet.prototype.updateFleetState(
              userFleets[i],
              "Mine [" +
                window.utils.timeUtils.TimeToStr(
                  new Date(Date.now() + miningDuration * 1000)
                ) +
                "]"
            );
      ...
      if (userFleets[i].state.slice(0, 4) === "Mine" && fleetMining) {
        ...
        let mineItem = await sageProgram.account.mineItem.fetch(
    ...
    async handleMineMovement(i, fleetCoords) {
      if (userFleets[i].moveTarget && userFleets[i].moveTarget !== "") {
        ...
        window.Fleet.prototype.updateFleetState(userFleets[i], msg);
      }
    }
  }

  class Movement {
    calculateMovementDistance(orig, dest) {
      return dest
        ? Math.sqrt((orig[0] - dest[0]) ** 2 + (orig[1] - dest[1]) ** 2)
        : 0;
    }
    ...
    async handleMovement(i, moveDist, moveX, moveY) {
      ...
      if (
        ...
        if (extra[0] !== moveX || extra[1] !== moveY) {
          let warpCost = window.Fleet.prototype.calcWarpFuelReq(
            userFleets[i],
            extra,
            [moveX, moveY]
          );
          ...
          let subwarpCost = window.Fleet.prototype.calculateSubwarpFuelBurn(
            userFleets[i],
            moveDist
          );
          let fleetCurrentFuelTank =
          ...
          let currentCargoFuelCnt = currentCargoFuel
            ? currentCargoFuel.account.data.parsed.info.tokenAmount.uiAmount
            : 0;

          //Should a warp be attempted?
          if (
            userFleets[i].moveType == "warp" &&
            currentFuelCnt + currentCargoFuelCnt >= warpCost
          ) {
            let fleetAcctData = sageProgram.coder.accounts.decode(
              "fleet",
              fleetAcctInfo.data
            );
            let warpCooldownExpiresAt =
              fleetAcctData.warpCooldownExpiresAt.toNumber() * 1000;

            //Wait for cooldown
            while (Date.now() < warpCooldownExpiresAt) {
              if (!userFleets[i].state.includes("Warp C/D")) {
                const warpCDExpireTimeStr = `[${window.utils.timeUtils.TimeToStr(
                ...
                window.logger.cLog(
                  ...
                  `${window.utils.timeUtils.FleetTimeStamp(
                    userFleets[i].label
                  )} Awaiting Warp C/D ${warpCDExpireTimeStr}`
                );
                this.updateFleetState(
                  userFleets[i],
                  `Warp C/D ${warpCDExpireTimeStr}`
                );
              }

              await window.utils.timeUtils.wait(
                Math.max(1000, warpCooldownExpiresAt - Date.now())
              );
            }
            await window.utils.timeUtils.wait(2000); //Extra wait to ensure accuracy
            ...
            if (moveDist > userFleets[i].maxWarpDistance / 100) {
              [moveX, moveY] = calcNextWarpPoint(
                userFleets[i].maxWarpDistance,
                extra,
                [moveX, moveY]
              );
              ...
              const fleetParsedData = JSON.parse(fleetSavedData);
              //window.logger.cLog(3, `${window.utils.timeUtils.FleetTimeStamp(userFleets[i].label)} moveTargets`, fleetParsedData.moveTarget, userFleets[i].moveTarget);
              ...
              moveDist = this.calculateMovementDistance(extra, [moveX, moveY]);
            }

            moveTime = this.calculateWarpTime(userFleets[i], moveDist);
            const warpResult = await execWarp(
              userFleets[i],
              moveX,
              moveY,
              moveTime
            ...
            moveTime = window.Fleet.prototype.calculateSubwarpTime(
              userFleets[i],
              moveDist
            );
            await window.Fleet.prototype.execSubwarp(
              userFleets[i],
              moveX,
              moveY,
              moveTime
            ...
            this.updateFleetState(userFleets[i], "ERROR: Not enough fuel");
          }
        }
      }

      await window.utils.timeUtils.wait(2000); //Allow time for RPC to update
      fleetAcctInfo = await window.assistant.getAccountInfo(
        userFleets[i].label,
        "full fleet info",
        userFleets[i].publicKey
      );
      [fleetState, extra] = window.Fleet.prototype.getFleetState(fleetAcctInfo);
      let warpFinish =
      ...
      window.logger.cLog(
        ...
        `${window.utils.timeUtils.FleetTimeStamp(
          userFleets[i].label
        )} Expected arrival (chain): ${window.utils.timeUtils.TimeToStr(
          new Date(endTime)
        )}`
      );
      window.logger.cLog(
        ...
        `${window.utils.timeUtils.FleetTimeStamp(
          userFleets[i].label
        )} Expected arrival (calc): ${window.utils.timeUtils.TimeToStr(
          new Date(calcEndTime)
        )}`
      );
      ...
      await window.utils.timeUtils.wait(moveTime * 1000);
      while (endTime > Date.now()) {
        const newFleetState =
          "Move [" + window.utils.timeUtils.TimeToStr(new Date(endTime)) + "]";
        window.Fleet.prototype.updateFleetState(userFleets[i], newFleetState);
        await window.utils.timeUtils.wait(Math.max(1000, endTime - Date.now()));
      }

      //await window.utils.timeUtils.wait(2000);
      if (fleetState == "MoveWarp") {
        await window.Fleet.prototype.execExitWarp(userFleets[i]);
      } else if (fleetState == "MoveSubwarp") {
        await window.Fleet.prototype.execExitSubwarp(userFleets[i]);
      }

      fleetAcctInfo = await window.blockchainManager.getAccountInfo(
        userFleets[i].label,
        "full fleet info",
        userFleets[i].publicKey
      );
  ...
  class ProxyManager {
    ...
    async doProxyStuff(target, origMethod, args, rpcs, proxyType) {
      ...
      try {
        ...
        window.logger.cLog(2, `${proxyType} CONNECTION ERROR: `, error1);
        window.logger.cLog(
          2,
  ...
  class Scanning {
    buildScanBlock(destX, destY) {
      ...
      if (scanBlockPattern == "ring") {
        ...
        for (let i = 0; i < scanBlockLength; i++)
          scanBlock.push([destX, destY + i]);
        for (let i = 0; i < scanBlockLength; i++)
          scanBlock.push([destX + 1, destY + (tip - i)]);
        ...
        for (let i = 0; i < scanBlockLength; i++)
          scanBlock.push([destX - i, destY]);
        for (let i = 0; i < scanBlockLength; i++)
          scanBlock.push([destX - (tip - i), destY + 1]);
    ...
    async execScan(fleet, fleetCoords) {
      return new Promise(async (resolve) => {
        let sector = await window.Starbase.prototype.getSectorFromCoords(fleetCoords[0], fleetCoords[1]);
        try {
          ...
          let txDiscover = {
            instruction: await sageProgram.methods
              .discoverSector({
                coordinates: [
                  new BrowserAnchor.anchor.BN(fleetCoords[0]),
                  new BrowserAnchor.anchor.BN(fleetCoords[1]),
                ],
                keyIndex: new BrowserAnchor.anchor.BN(userProfileKeyIdx),
              })
              .accountsStrict({
          ...
          window.Fleet.prototype.updateFleetState(
            fleet,
            `Scanning [${window.utils.timeUtils.TimeToStr(
              new Date(Date.now())
            )}]`
          );
          let txResult = await window.blockchainManager.txSignAndSend(
        ...
        let sduCargoTypeAcct = cargoTypes.find(
          (item) => item.account.mint.toString() == sduItem.token
        );
        let foodCargoTypeAcct = cargoTypes.find(
          (item) =>
            item.account.mint.toString() == sageGameAcct.account.mints.food
        ...
        window.Fleet.prototype.updateFleetState(
          fleet,
          `Scanning [${window.utils.timeUtils.TimeToStr(new Date(Date.now()))}]`
        );
    ...
    async handleScan(i, fleetCoords, destCoords) {
      ...
      let currentFoodAcct = fleetCurrentCargo.value.find(
        (item) => item.pubkey.toString() === userFleets[i].foodToken.toString()
      );
      let currentFoodCnt =
        currentFoodAcct &&
        currentFoodAcct.account.data.parsed.info.delegatedAmount
          ? currentFoodAcct.account.data.parsed.info.delegatedAmount.uiAmount
          : 0;

      if (
        //Cargo full check for data runner fleets
        (userFleets[i].scanCost == 0 &&
          userFleets[i].cargoCapacity - cargoCnt < userFleets[i].sduPerScan) ||
      ...
      let moved = false;
      if (
        fleetCoords[0] !== destCoords[0] ||
        fleetCoords[1] !== destCoords[1]
      ) {
        if (!userFleets[i].state.includes("Warp C/D")) {
          const starbaseCoords = window.utils.coordinateUtils.ConvertCoords(
            userFleets[i].starbaseCoord
          );
          ...
          if (userFleets[i].moveType == "warp") {
            ...
            fuelNeeded = window.Fleet.prototype.calculateSubwarpFuelBurn(
              userFleets[i],
          ...
          const currentFuelCnt = currentFuel
            ? currentFuel.account.data.parsed.info.tokenAmount.uiAmount
            : 0;
          const fuelReadout = `Fuel: need ${Math.round(
            fuelNeeded
          )} / have ${Math.round(currentFuelCnt)}`;
          if (currentFuelCnt > fuelNeeded) {
            window.logger.cLog(
              1,
              `${window.utils.timeUtils.FleetTimeStamp(userFleets[i].label)}`,
              fuelReadout
            );
            let moveDist = window.Movement.prototype.calculateMovementDistance(
              fleetCoords,
            ...
            if (moveDist > 0) {
              ...
              userFleets[i].scanEnd =
                scanEndsIn > userFleets[i].scanCooldown
                  ? userFleets[i].scanCooldown
                  : scanEndsIn;
              await window.Movement.prototype.handleMovement(
                i,
                moveDist,
      ...
      if (!moved && Date.now() > userFleets[i].scanEnd) {
        ...
        const changesSDU = scanResult
          ? window.blockchainManager.getBalanceChange(scanResult, userFleets[i].sduToken.toString())
          : {
              postBalance: userFleets[i].sduCnt,
              preBalance: userFleets[i].sduCnt,
            };
        const changesFood = scanResult
          ? window.blockchainManager.getBalanceChange(scanResult, userFleets[i].foodToken.toString())
          : {
              postBalance: userFleets[i].foodCnt - userFleets[i].scanCost,
              preBalance: userFleets[i].foodCnt,
            };
        const scanConditionLog =
          scanResult && scanResult.meta.logMessages
            ? scanResult.meta.logMessages.find((item) =>
                item.startsWith("Program log: SDU probability:")
              )
            : null;
        const scanCondition = scanConditionLog
          ? (Number(scanConditionLog.split(" ").pop()) * 100).toFixed(4)
          : 0;
        ...
        if (userFleets[i].scanMove && struckOut)
          userFleets[i].scanBlockIdx =
            userFleets[i].scanBlockIdx > userFleets[i].scanBlock.length - 2
              ? 0
              : userFleets[i].scanBlockIdx + 1;

        const needPause =
          (struckOut && !userFleets[i].scanMove) ||
          (userFleets[i].scanMove &&
            userFleets[i].scanSkipCnt >= userFleets[i].scanBlock.length - 1);
        ...
        if (needPause) {
          ...
          userFleets[
            i
          ].state = `Scanning Paused [${window.utils.timeUtils.TimeToStr(
          ...
          window.logger.cLog(
            ...
            `${window.utils.timeUtils.FleetTimeStamp(
              userFleets[i].label
            )} Scanning Paused due to low probability [${window.utils.timeUtils.TimeToStr(
              new Date(userFleets[i].scanEnd)
            )}]`
          );
          userFleets[i].scanSkipCnt = 0;
      ...
      if (!userFleets[i].startupScanBlockCheck && fleetCoords.length > 0) {
        ...
        if (userFleets[i].scanMove) {
          ...
          for (let s = 0; s < userFleets[i].scanBlock.length - 1; s++) {
            ...
            if (
              fleetCoords[0] == testCoords[0] &&
              fleetCoords[1] == testCoords[1]
            ) {
  ...
  class Settings {
    ...
    async loadGlobalSettings() {
      ...
      window.logger.cLog(2, "SYSTEM: Global Settings loaded", globalSettings);
    }
    ...
    async saveSettingsInput() {
      ...
      globalSettings = {
        ...
        savedProfile: saveProfile
          ? userProfileAcct && userProfileKeyIdx
            ? [userProfileAcct.toString(), userProfileKeyIdx]
            : []
          : [],
        confirmationCheckingDelay: window.utils.typeUtils.parseIntDefault(
        ...
        transportUseAmmoBank: document.querySelector("#transportUseAmmoBank")
          .checked,
        transportStopOnError: document.querySelector("#transportStopOnError")
          .checked,
        ...
        scanBlockResetAfterResupply: document.querySelector(
          "#scanBlockResetAfterResupply"
        ).checked,
        scanResupplyOnLowFuel: document.querySelector("#scanResupplyOnLowFuel")
          .checked,
        scanSectorRegenTime: window.utils.typeUtils.parseIntDefault(
  ...
  class Starbase {
    async execCargoFromStarbaseToFleet(
      ...
      return new Promise(async (resolve) => {
        ...
        let starbaseX = dockCoords.split(",")[0].trim();
        let starbaseY = dockCoords.split(",")[1].trim();
        let starbase = await this.getStarbaseFromCoords(starbaseX, starbaseY);
        let starbasePlayer = await this.getStarbasePlayer(
          userProfileAcct,
          starbase.publicKey
        );
        let starbasePlayerCargoHolds = await cargoProgram.account.cargoPod.all([
        ...
        for (let cargoHold of starbasePlayerCargoHolds) {
          if (cargoHold.account && cargoHold.account.openTokenAccounts > 0) {
            ...
            let cargoHoldFound = cargoHoldTokens.value.find(
              (item) =>
                item.account.data.parsed.info.mint === tokenMint &&
                item.account.data.parsed.info.tokenAmount.uiAmount >= amount
            );
            if (cargoHoldFound) {
              starbasePlayerCargoHold = cargoHold;
              mostFound =
                cargoHoldFound.account.data.parsed.info.tokenAmount.uiAmount;
              break;
            } else {
              let cargoHoldFound = cargoHoldTokens.value.find(
                (item) =>
        ...
        if (amount > 0) {
            ...
            instruction: await sageProgram.methods
              ...
              .accountsStrict({
                ...
                cargoStatsDefinition:
                  sageGameAcct.account.cargo.statsDefinition,
          ...
          //Send tx
          txResult = await window.blockchainManager.txSignAndSend(
            tx,
            fleet,
            "LOAD",
            100
    ...
    async execRegisterStarbasePlayer(fleet, dockCoords) {
      let starbaseX = dockCoords.split(",")[0].trim();
      let starbaseY = dockCoords.split(",")[1].trim();
      let starbase = await this.getStarbaseFromCoords(starbaseX, starbaseY);

      let [sagePlayerProfile] =
        await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
          [
    ...
    async getStarbaseFromCoords(x, y) {
      return new Promise(async (resolve) => {
        let xBN = new BrowserAnchor.anchor.BN(x);
        let xArr = xBN
          .toTwos(64)
    ...
    async getStarbaseUpkeepStatus() {
      let starbases = await sageProgram.account.starbase.all();
      let factionStarbases = starbases.filter(
        (sb) => sb.account.faction == userProfileFactionAcct.account.faction
      );
      let starbasesStatus = [];
      for (let starbase of factionStarbases) {
        starbasesStatus.push(await this.upkeepBalance(starbase));
      }
      return starbasesStatus;
    }

    async upkeepBalance(starbase) {
      let gameStateAcct = await sageProgram.account.gameState.fetch(
        sageGameAcct.account.gameState
      );
      let sbLevel =
        gameStateAcct.fleet.upkeep["level" + starbase.account.level];
      let upkeepFoodBalance = Math.max(
        0,
        starbase.account.upkeepFoodBalance.toNumber() -
          (Math.floor(Date.now() / 1000) -
            starbase.account.upkeepFoodGlobalLastUpdate.toNumber()) *
            (sbLevel.foodDepletionRate / 100)
      );
      let foodBalanceHr =
        upkeepFoodBalance / (sbLevel.foodDepletionRate / 100) / 60 / 60;
      let upkeepToolBalance = Math.max(
        0,
        starbase.account.upkeepToolkitBalance.toNumber() -
          (Math.floor(Date.now() / 1000) -
            starbase.account.upkeepToolkitGlobalLastUpdate.toNumber()) *
            (sbLevel.toolkitDepletionRate / 100)
      );
      let toolBalanceHr =
        upkeepToolBalance / (sbLevel.toolkitDepletionRate / 100) / 60 / 60;
      let [name] = new TextDecoder()
        .decode(new Uint8Array(starbase.account.name))
        .replace(/\0/g, "")
        .split(" (");
      name = name === "ONI Central Space Station" ? "ONI CSS" : name;
      let coords =
        starbase.account.sector[0].toNumber() +
        ", " +
        starbase.account.sector[1].toNumber();
      return {
        name: `${name} [LVL ${starbase.account.level}]`,
        coords: coords,
        foodBalanceHr: foodBalanceHr.toFixed(2),
        foodBalancePerc: (upkeepFoodBalance / sbLevel.foodReserve).toFixed(2),
        toolBalanceHr: toolBalanceHr.toFixed(2) || "N/A",
        toolBalancePerc:
          (upkeepToolBalance / sbLevel.toolkitReserve).toFixed(2) || "N/A",
      };
    }
  }
  ...
  class Transport {
    async handleTransport(i, fleetState, fleetCoords) {
      const [destX, destY] = window.utils.coordinateUtils.ConvertCoords(
        userFleets[i].destCoord
      );
      const [starbaseX, starbaseY] = window.utils.coordinateUtils.ConvertCoords(
        userFleets[i].starbaseCoord
      );
      ...
      let targetCargoManifest = [
        ...
        {
          res: fleetParsedData.transportResource2,
          amt: fleetParsedData.transportResource2Perc,
        },
        {
          res: fleetParsedData.transportResource3,
          amt: fleetParsedData.transportResource3Perc,
        },
        {
          res: fleetParsedData.transportResource4,
          amt: fleetParsedData.transportResource4Perc,
        },
      ];
      let starbaseCargoManifest = [
        ...
        {
          res: fleetParsedData.transportSBResource2,
          amt: fleetParsedData.transportSBResource2Perc,
        },
        {
          res: fleetParsedData.transportSBResource3,
          amt: fleetParsedData.transportSBResource3Perc,
        },
        {
          res: fleetParsedData.transportSBResource4,
          amt: fleetParsedData.transportSBResource4Perc,
        },
      ];
      const hasTargetManifest = this.hasTransportManifest(targetCargoManifest);
      const hasStarbaseManifest = this.hasTransportManifest(
      ...
      if (fleetState === "Idle") {
        ...
        if (fleetCoords[0] == starbaseX && fleetCoords[1] == starbaseY) {
          ...
          let fuelIndex = targetCargoManifest.findIndex(
            (e) => e.res === sageGameAcct.account.mints.fuel.toString()
          );
          if (fuelIndex > -1) {
            targetCargoManifest[fuelIndex].amt =
              targetCargoManifest[fuelIndex].amt - refuelResp.amount;
          }

          //Loading at Starbase
          if (hasTargetManifest) {
            const loadedCargo = await handleTransportLoading(
              i,
              userFleets[i].starbaseCoord,
              targetCargoManifest
            );
        ...
        // Fleet at target?
          ...
          let fuelIndex = starbaseCargoManifest.findIndex(
            (e) => e.res === sageGameAcct.account.mints.fuel.toString()
          );
          if (fuelIndex > -1) {
            starbaseCargoManifest[fuelIndex].amt =
              starbaseCargoManifest[fuelIndex].amt - refuelResp.amount;
          }
        ...
        if (userFleets[i].moveTarget !== "") {
          ...
          window.Fleet.prototype.updateFleetState(
            userFleets[i],
            "ERROR: Fleet must start at Target or Starbase"
          );
        }
      }
    }

    hasTransportManifest(manifest) {
      for (const entry of manifest) if (entry.res && entry.amt) return true;
  ...
  class User {
    buildXpAccounts(xpCategory, userXpAccounts, userXpAccountGroup) {
      return new Promise(async (resolve) => {
        let [userXpAccount] =
          await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
            [
            ...
            window.blockchainManager.pointsProgramId
          );

        let [pointsModifierAccount] =
          await pointsProgram.account.pointsModifier.all([
            {
              memcmp: {
                offset: 9,
  ...
  class UserInterfaceManager {
    constructor(observer) {
      ...
      assistSettingsButton.addEventListener("click", (e) => {
      ...
      assistConfigButton.addEventListener("click", (e) => {
      ...
      assistCheckButton.addEventListener("click", (e) => {
      ...
      assistStatusButton.addEventListener("click", (e) => {
      ...
      assistStarbaseStatusButton.addEventListener("click", (e) => {
      ...
      assistModalClose.addEventListener("click", (e) => {
        this.assistModalToggle();
      });
      let assistModalSave = document.querySelector(
        "#assistModal .assist-modal-save"
      );
      assistModalSave.addEventListener("click", (e) => {
      ...
      configImportClose.addEventListener("click", (e) => {
        this.assistImportToggle();
      });
    ...
    async addAssistInput(fleet) {
      ...
      assistAssignments.forEach(function (assignment) {
        assignmentOptionsStr +=
          '<option value="' + assignment + '">' + assignment + "</option>";
      });
      fleetAssignment.innerHTML = assignmentOptionsStr;
      fleetAssignment.value =
        fleetParsedData && fleetParsedData.assignment
          ? fleetParsedData.assignment
          : "";
      ...
      fleetSubwarpPref.checked =
        fleetParsedData &&
        fleetParsedData.subwarpPref &&
        fleetParsedData.subwarpPref == "true"
          ? true
          : false;
      ...
      let targetElem = document.querySelector(
        "#assistModal .assist-modal-body table"
      );
      ...
      scanRow.style.display =
        fleetParsedData && fleetParsedData.assignment == "Scan"
          ? "table-row"
          : "none";
      fleetParsedData &&
        fleetParsedData.assignment == "Scan" &&
      ...
      scanMove.checked =
        (fleetParsedData &&
          ...
          fleetParsedData.scanMove == "false") ||
        false
          ? false
          : true;
...
*/
      ...
      mineRow.style.display =
        fleetParsedData && fleetParsedData.assignment == "Mine"
          ? "table-row"
          : "none";
      fleetParsedData &&
        fleetParsedData.assignment == "Mine" &&
      ...
      let resourceToken =
        ...
        fleetParsedData.mineResource &&
        fleetParsedData.mineResource !== ""
          ? cargoItems.find((r) => r.token == fleetParsedData.mineResource)
          : "";
      fleetMineRes.value =
        resourceToken && resourceToken.name ? resourceToken.name : "";
      ...
      transportRow.style.display =
        fleetParsedData && fleetParsedData.assignment == "Transport"
          ? "table-row"
          : "none";
      fleetParsedData &&
        fleetParsedData.assignment == "Transport" &&
      ...
      let transportOptStr = "";
      transportResources.forEach(function (resource) {
        transportOptStr +=
          '<option value="' + resource + '">' + resource + "</option>";
      });
      let transportResource1 = document.createElement("select");
      transportResource1.innerHTML = transportOptStr;
      let transportResource1Token =
        fleetParsedData &&
        fleetParsedData.transportResource1 &&
        fleetParsedData.transportResource1 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportResource1
            )
          : "";
      transportResource1.value =
        transportResource1Token && transportResource1Token.name
          ? transportResource1Token.name
          : "";
      ...
      transportResource1Perc.value =
        fleetParsedData && fleetParsedData.transportResource1Perc
          ? fleetParsedData.transportResource1Perc
          : "";
      ...
      let transportResource2Token =
        fleetParsedData &&
        fleetParsedData.transportResource2 &&
        fleetParsedData.transportResource2 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportResource2
            )
          : "";
      transportResource2.value =
        transportResource2Token && transportResource2Token.name
          ? transportResource2Token.name
          : "";
      ...
      transportResource2Perc.value =
        fleetParsedData && fleetParsedData.transportResource2Perc
          ? fleetParsedData.transportResource2Perc
          : "";
      ...
      let transportResource3Token =
        fleetParsedData &&
        fleetParsedData.transportResource3 &&
        fleetParsedData.transportResource3 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportResource3
            )
          : "";
      transportResource3.value =
        transportResource3Token && transportResource3Token.name
          ? transportResource3Token.name
          : "";
      ...
      transportResource3Perc.value =
        fleetParsedData && fleetParsedData.transportResource3Perc
          ? fleetParsedData.transportResource3Perc
          : "";
      ...
      let transportResource4Token =
        fleetParsedData &&
        fleetParsedData.transportResource4 &&
        fleetParsedData.transportResource4 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportResource4
            )
          : "";
      transportResource4.value =
        transportResource4Token && transportResource4Token.name
          ? transportResource4Token.name
          : "";
      ...
      transportResource4Perc.value =
        fleetParsedData && fleetParsedData.transportResource4Perc
          ? fleetParsedData.transportResource4Perc
          : "";
      ...
      let transportSBResource1Token =
        fleetParsedData &&
        fleetParsedData.transportSBResource1 &&
        fleetParsedData.transportSBResource1 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportSBResource1
            )
          : "";
      transportSBResource1.value =
        transportSBResource1Token && transportSBResource1Token.name
          ? transportSBResource1Token.name
          : "";
      ...
      transportSBResource1Perc.value =
        fleetParsedData && fleetParsedData.transportSBResource1Perc
          ? fleetParsedData.transportSBResource1Perc
          : "";
      ...
      let transportSBResource2Token =
        fleetParsedData &&
        fleetParsedData.transportSBResource2 &&
        fleetParsedData.transportSBResource2 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportSBResource2
            )
          : "";
      transportSBResource2.value =
        transportSBResource2Token && transportSBResource2Token.name
          ? transportSBResource2Token.name
          : "";
      ...
      transportSBResource2Perc.value =
        fleetParsedData && fleetParsedData.transportSBResource2Perc
          ? fleetParsedData.transportSBResource2Perc
          : "";
      ...
      let transportSBResource3Token =
        fleetParsedData &&
        fleetParsedData.transportSBResource3 &&
        fleetParsedData.transportSBResource3 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportSBResource3
            )
          : "";
      transportSBResource3.value =
        transportSBResource3Token && transportSBResource3Token.name
          ? transportSBResource3Token.name
          : "";
      ...
      transportSBResource3Perc.value =
        fleetParsedData && fleetParsedData.transportSBResource3Perc
          ? fleetParsedData.transportSBResource3Perc
          : "";
      ...
      let transportSBResource4Token =
        fleetParsedData &&
        fleetParsedData.transportSBResource4 &&
        fleetParsedData.transportSBResource4 !== ""
          ? cargoItems.find(
              (r) => r.token == fleetParsedData.transportSBResource4
            )
          : "";
      transportSBResource4.value =
        transportSBResource4Token && transportSBResource4Token.name
          ? transportSBResource4Token.name
          : "";
      ...
      transportSBResource4Perc.value =
        fleetParsedData && fleetParsedData.transportSBResource4Perc
          ? fleetParsedData.transportSBResource4Perc
          : "";
      ...
      padRow.style.display =
        fleetParsedData && fleetParsedData.assignment ? "table-row" : "none";
    ...
    async assistImportToggle() {
      ...
      if (targetElem.style.display === "none") {
        ...
        //window.logger.cLog(2, 'assistImportToggle: fleetKeys', fleetKeys);
    ...
    assistModalToggle() {
      window.logger.cLog(
        ...
        `${window.utils.timeUtils.FleetTimeStamp(
          "SYSTEM"
        )} Solana Reads: ${solanaReadCount} / Writes: ${solanaWriteCount}`
      );
      let targetElem = document.querySelector("#assistModal");
      if (targetElem.style.display === "none") {
        document
          .querySelectorAll("#assistModal .assist-fleet-row")
          .forEach((e) => e.remove());
        document
          .querySelectorAll("#assistModal .assist-scan-row")
          .forEach((e) => e.remove());
        document
          .querySelectorAll("#assistModal .assist-mine-row")
          .forEach((e) => e.remove());
        document
          .querySelectorAll("#assistModal .assist-pad-row")
          .forEach((e) => e.remove());
        document
          .querySelectorAll("#assistModal .assist-transport-row")
          .forEach((e) => e.remove());
        for (let fleet of userFleets) this.addAssistInput(fleet);
        targetElem.style.display = "block";
      } else {
        targetElem.style.display = "none";
      }
    ...
    async assistProfileToggle(profiles) {
      return new Promise(async (resolve) => {
        ...
        if (targetElem.style.display === "none" && profiles) {
          ...
          profileSelect.onchange = function () {
            ...
            let selected = profiles.find(
              (o) => o.profile === profileSelect.value
            );
    ...
    async assistStarbaseStatusToggle() {
      let targetElem = document.querySelector("#assistStarbaseStatus");
      if (targetElem.style.display === "none") {
        let starbases = await window.Starbase.prototype.getStarbaseUpkeepStatus();
        this.updateAssistStarbaseStatus(starbases);
        targetElem.style.display = "block";
      } else {
        targetElem.style.display = "none";
      }
    ...
    async saveAssistInput() {
      ...
      let transportRows = document.querySelectorAll(
        "#assistModal .assist-transport-row > td"
      );
      ...
      for (let [i, row] of fleetRows.entries()) {
        ...
        let fleetStarbaseCoord =
          window.utils.coordinateUtils.validateCoordInput(
        ...
        const destCoords =
          window.utils.coordinateUtils.ConvertCoords(fleetDestCoord);
        const starbaseCoords =
          window.utils.coordinateUtils.ConvertCoords(fleetStarbaseCoord);

        if (fleetAssignment !== "") {
          //let warpCost = calculateWarpFuelBurn(userFleets[userFleetIndex], moveDist);
          let warpCost = window.Fleet.prototype.calcWarpFuelReq(
            userFleets[userFleetIndex],
            starbaseCoords,
            destCoords
          );
          if (warpCost > userFleets[userFleetIndex].fuelCapacity) {
            ...
            if (subwarpCost * 2 > userFleets[userFleetIndex].fuelCapacity) {
              inputError(
                "ERROR: Fleet will not have enough fuel to return to starbase",
                "ERROR: Distance exceeds fuel capacity"
              );
            } else {
              moveType = "subwarp";
            }
        ...
        let scanMin =
          parseInt(scanRows[i].children[1].children[0].children[1].value) || 0;
        ...
        fleetMineResource =
          fleetMineResource !== ""
            ? cargoItems.find((r) => r.name == fleetMineResource).token
            : "";

        let transportToTarget = transportRows[i].querySelectorAll(
          ":scope > .transport-to-target > div"
        );
        let transportResource1 = transportToTarget[1].children[0].value;
        transportResource1 =
          transportResource1 !== ""
            ? cargoItems.find((r) => r.name == transportResource1).token
            : "";
        let transportResource1Perc =
          parseInt(transportToTarget[1].children[1].value) || 0;
        let transportResource2 = transportToTarget[2].children[0].value;
        transportResource2 =
          transportResource2 !== ""
            ? cargoItems.find((r) => r.name == transportResource2).token
            : "";
        let transportResource2Perc =
          parseInt(transportToTarget[2].children[1].value) || 0;
        let transportResource3 = transportToTarget[3].children[0].value;
        transportResource3 =
          transportResource3 !== ""
            ? cargoItems.find((r) => r.name == transportResource3).token
            : "";
        let transportResource3Perc =
          parseInt(transportToTarget[3].children[1].value) || 0;
        let transportResource4 = transportToTarget[4].children[0].value;
        transportResource4 =
          transportResource4 !== ""
            ? cargoItems.find((r) => r.name == transportResource4).token
            : "";
        let transportResource4Perc =
          parseInt(transportToTarget[4].children[1].value) || 0;

        let transportToStarbase = transportRows[i].querySelectorAll(
          ":scope > .transport-to-starbase > div"
        );
        let transportSBResource1 = transportToStarbase[1].children[0].value;
        transportSBResource1 =
          transportSBResource1 !== ""
            ? cargoItems.find((r) => r.name == transportSBResource1).token
            : "";
        let transportSBResource1Perc =
          parseInt(transportToStarbase[1].children[1].value) || 0;
        let transportSBResource2 = transportToStarbase[2].children[0].value;
        transportSBResource2 =
          transportSBResource2 !== ""
            ? cargoItems.find((r) => r.name == transportSBResource2).token
            : "";
        let transportSBResource2Perc =
          parseInt(transportToStarbase[2].children[1].value) || 0;
        let transportSBResource3 = transportToStarbase[3].children[0].value;
        transportSBResource3 =
          transportSBResource3 !== ""
            ? cargoItems.find((r) => r.name == transportSBResource3).token
            : "";
        let transportSBResource3Perc =
          parseInt(transportToStarbase[3].children[1].value) || 0;
        let transportSBResource4 = transportToStarbase[4].children[0].value;
        transportSBResource4 =
          transportSBResource4 !== ""
            ? cargoItems.find((r) => r.name == transportSBResource4).token
            : "";
        let transportSBResource4Perc =
          parseInt(transportToStarbase[4].children[1].value) || 0;
        ...
        if (rowErrBool === false) {
          ...
          let fleetMoveTarget =
            fleetParsedData && fleetParsedData.moveTarget
              ? fleetParsedData.moveTarget
              : "";
          let scanBlock = window.Scanning.prototype.buildScanBlock(
            destCoords[0],
            destCoords[1]
          );
          ...
          userFleets[userFleetIndex].scanBlockIdx = scanMove
            ? userFleets[userFleetIndex].scanBlockIdx
            : 0;
        }
      }
    ...
    async updateAssistStarbaseStatus(starbases) {
      document
        .querySelectorAll(
          "#assistStarbaseStatus .assist-modal-body table assist-starbase-row"
        )
        .forEach((e) => e.remove());
      ...
      for (let starbase of starbases) {
        ...
        starbaseFood.innerHTML = `${starbase.foodBalanceHr}hr [${(
          starbase.foodBalancePerc * 100
        ).toFixed(2)}%]`;
        ...
        starbaseTools.innerHTML = `${starbase.toolBalanceHr}hr [${(
          starbase.toolBalancePerc * 100
        ).toFixed(2)}%]`;
        ...
        let targetElem = document.querySelector(
          "#assistStarbaseStatus .assist-modal-body table"
        );
    ...
    updateAssistStatus(fleet) {
      let targetRow = document.querySelectorAll(
        '#assistStatus .assist-fleet-row[pk="' +
          fleet.publicKey.toString() +
          '"]'
      );
      ...
      if (
        targetRow &&
        targetRow.length > 0 &&
        targetRow[0].children &&
        targetRow[0].children.length > 0
      )
        targetRow[0].children[0].firstChild.style.color = fleet.fontColor
          ? fleet.fontColor
          : "white";
    }
  }
    ...
    get(target, key, receiver) {
      ...
      if (typeof origMethod === "function") {
        return async function (...args) {
          solanaReadCount++;
          return await window.proxyManager.doProxyStuff(
            target,
            origMethod,
            args,
            window.blockchainManager.readRPCs,
            "READ"
          );
        };
      }
      ...
      if (typeof origMethod === "function") {
        return async function (...args) {
          ...
          return await window.proxyManager.doProxyStuff(
            ...
            origMethod,
            args,
            window.blockchainManager.writeRPCs,
            "WRITE"
          );
        };
      }
  ...
  window.utils = {
    ...
    timeUtils: {
      ...
      TimeStamp: function () {
        return `[${this.TimeToStr(new Date(Date.now()))}]`;
      },
      FleetTimeStamp: function (fleetName) {
        return `[${fleetName}] ${this.TimeStamp()}`;
      },
    },
    typeUtils: {
      ...
      parseBoolDefault: function (value, defaultValue) {
  ...
  window.logger.cLog(
    1,
    `Read RPC: ${
      window.blockchainManager.readRPCs[window.blockchainManager.readIdx]
    }`
  );
  window.logger.cLog(
    1,
    `Write RPC: ${
      window.blockchainManager.writeRPCs[window.blockchainManager.writeIdx]
    }`
  );

  //Not sure what this does, but it seems to do some reads, so sticking it on the read connection
  const anchorProvider = new BrowserAnchor.anchor.AnchorProvider(
    window.blockchainManager.solanaReadConnection,
    null,
    null
  );
  ...
  const sageIDL = {
    ...
    instructions: [
      {
        ...
        args: [{ name: "input", type: { defined: "ManageGameInput" } }],
      },
      {
        name: "addConnection",
        accounts: [
        ...
        args: [
          { name: "subCoordinates1", type: { array: ["i64", 2] } },
          { name: "flags1", type: "u8" },
          { name: "subCoordinates2", type: { array: ["i64", 2] } },
          { name: "flags2", type: "u8" },
      ...
      },
      {
        ...
        args: [{ name: "ownerKeyIndex", type: "u16" }],
      },
      {
        name: "addShipEscrow",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "AddShipEscrowInput" } }],
      },
      {
        name: "addShipToFleet",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "AddShipToFleetInput" } }],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "IngredientIndexInput" } }],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "KeyIndexInput" } }],
      },
      {
        name: "changeRental",
        accounts: [
          ...
          { name: "fleet", isMut: !0, isSigner: !1, docs: ["The fleet"] },
          {
            name: "gameAccounts",
            accounts: [
              {
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "CloseDisbandedFleetInput" } },
        ],
      },
      {
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "StarbaseCreateCargoPodInput" } },
        ],
      },
      {
        ...
        accounts: [
          ...
          { name: "rent", isMut: !1, isSigner: !1, docs: ["The rent sysvar"] },
          {
            name: "tokenProgram",
            isMut: !1,
            isSigner: !1,
      ...
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "StarbaseCreateCraftingProcessInput" },
          },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "CreateFleetInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "DepositCargoToFleetInput" } },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "CargoToGameInput" } }],
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "StarbaseDepositCraftingIngredientInput" },
          },
        ],
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "DepositStarbaseUpkeepResourceInput" },
          },
        ],
      },
      {
      ...
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "DeregisterSurveyDataUnitTrackerInput" },
          },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "DisbandFleetInput" } }],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "DisbandedFleetToEscrowInput" } },
        ],
      },
      {
        name: "discoverSector",
        ...
        args: [{ name: "input", type: { defined: "DiscoverSectorInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "ForcedDisbandFleetInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "IdleToRespawnInput" } }],
      },
      {
        name: "initGame",
        accounts: [
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "InitGameStateInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "MineAsteroidToRespawnInput" } },
        ],
      },
      {
      ...
      },
      {
        ...
        accounts: [
          ...
          },
          {
            ...
            isMut: !1,
            isSigner: !0,
            docs: ["Owner of the certificates"],
          },
          {
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "RegisterMineItemInput" } }],
      },
      {
        name: "registerPlanet",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "RegisterPlanetInput" } }],
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "RegisterProgressionConfigInput" },
          },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "RegisterResourceInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "RegisterSagePointsModifierInput" },
          },
        ],
      },
      {
        ...
        args: [
          { name: "coordinates", type: { array: ["i64", 2] } },
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "RegisterShipInput" } }],
      },
      {
        name: "registerStar",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "RegisterStarInput" } }],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "RegisterStarbaseInputUnpacked" } },
        ],
      },
      {
      ...
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "RegisterSurveyDataUnitTrackerInput" },
          },
        ],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "StarbaseRemoveCargoPodInput" } },
        ],
      },
      {
        ...
        args: [
          { name: "sector1Index", type: "u16" },
          { name: "sector2Index", type: "u16" },
          { name: "keyIndex", type: "u16" },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "RemoveShipEscrowInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "RespawnToLoadingBayInput" } },
        ],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "ScanForSurveyDataUnitsInput" } },
        ],
      },
      {
        name: "setNextShip",
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "StartSubwarpInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "StopMiningAsteroidInput" } }],
      },
      {
        name: "stopSubwarp",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "StopSubwarpInput" } }],
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "SubmitStarbaseUpgradeResourceInput" },
          },
        ],
      },
      {
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "StarbaseTransferCargoInput" } },
        ],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "TransferCargoWithinFleetInput" } },
        ],
      },
      {
        name: "updateGame",
        ...
        args: [{ name: "input", type: { defined: "UpdateGameInput" } }],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "UpdateGameStateInput" } }],
      },
      {
        name: "updateMineItem",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "UpdateMineItemInput" } }],
      },
      {
        name: "updatePlanet",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "UpdatePlanetInput" } }],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "UpdateProgressionConfigInput" } },
        ],
      },
      {
        name: "updateResource",
        ...
        args: [{ name: "input", type: { defined: "UpdateResourceInput" } }],
      },
      {
        name: "updateShip",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "UpdateShipInput" } }],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "UpdateShipEscrowInput" } }],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "UpdateShipFleetInput" } }],
      },
      {
        name: "updateStar",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "UpdateStarInput" } }],
      },
      {
        name: "updateStarbase",
        accounts: [
        ...
        args: [{ name: "input", type: { defined: "UpdateStarbaseInput" } }],
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "UpdateSurveyDataUnitTrackerInput" },
          },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "WarpLaneInput" } }],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "WarpToCoordinateInput" } }],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "WithdrawCargoFromFleetInput" } },
        ],
      },
      {
      ...
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "StarbaseWithdrawCraftingIngredientInput" },
          },
        ],
      },
    ],
    accounts: [
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "instanceType", docs: ["the planet type"], type: "u8" },
            {
              name: "numCrew",
              docs: ["The number of crew taking part in the crafting process"],
              type: "u64",
            },
            { name: "bump", docs: ["Bump of Account PDA"], type: "u8" },
          ],
        },
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "bump", docs: ["The disbanded fleet's bump."], type: "u8" },
          ],
        },
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            },
            {
              ...
              type: { defined: "OptionalNonSystemPubkey" },
            },
            {
              name: "subProfileInvalidator",
            ...
            { name: "bump", docs: ["The fleet's bump."], type: "u8" },
          ],
        },
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            },
            {
              name: "fleet",
              docs: ["Fleet settings"],
              type: { defined: "FleetInfo" },
            },
            {
              name: "misc",
              docs: ["Miscellaneous settings"],
              type: { defined: "MiscVariables" },
            },
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "bump", docs: ["bump for PDA"], type: "u8" },
          ],
        },
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "planetType", docs: ["the planet type"], type: "u8" },
            { name: "position", docs: ["the planet position"], type: "u8" },
            { name: "size", docs: ["size"], type: "u64" },
            { name: "maxHp", docs: ["maximum health"], type: "u64" },
            {
              name: "currentHealth",
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "gameId", docs: ["the game_id pubkey"], type: "publicKey" },
            {
              name: "location",
            ...
            { name: "locationType", docs: ["the location type"], type: "u8" },
            {
              name: "systemRichness",
              docs: [
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "sizeClass", docs: ["the ship's size class"], type: "u8" },
            {
              name: "stats",
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "starType", docs: ["the star type"], type: "u8" },
          ],
        },
      },
      {
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "updateId", docs: ["The `Game` update id"], type: "u64" },
            {
    ...
    types: [
      ...
      },
      {
        ...
        type: {
          kind: "struct",
          fields: [
            { name: "amount", docs: ["cargo amount"], type: "u64" },
            {
              name: "keyIndex",
              docs: ["the index of the key in the player profile"],
              type: "u16",
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "resourceType", docs: ["the resource type"], type: "u8" },
            {
              name: "resourceIndex",
              docs: [
            ...
            { name: "amount", docs: ["the amount"], type: "u64" },
          ],
        },
      },
      {
          ...
          fields: [
            {
              ...
              type: { array: [{ defined: "StarbaseLevelInfo" }, 7] },
            },
            {
              name: "oni",
          ...
          fields: [
            {
              name: "starbaseLevels",
              docs: ["`Starbase` levels discriminated by faction"],
              type: { defined: "FactionsStarbaseLevelInfo" },
            },
            {
              name: "upkeep",
              docs: ["`Starbase` upkeep discriminated by level"],
              type: { defined: "StarbaseUpkeepLevels" },
            },
            {
              name: "maxFleetSize",
          ...
          fields: [
            {
              ...
              type: {
                option: { vec: { defined: "StarbaseLevelInfoArrayInput" } },
              },
            },
            {
              ...
              docs: ["`Starbase` Level Info array"],
              type: {
                option: { vec: { defined: "StarbaseUpkeepInfoArrayInput" } },
              },
            },
            {
              name: "maxFleetSize",
      ...
      },
      {
        ...
        type: { kind: "enum", variants: [{ name: "Planet" }] },
      },
      {
      ...
      },
      {
        ...
        type: {
          kind: "struct",
          fields: [
            { name: "atlas", docs: ["ATLAS token mint"], type: "publicKey" },
            { name: "polis", docs: ["POLIS token mint"], type: "publicKey" },
            { name: "ammo", docs: ["ammunition"], type: "publicKey" },
            { name: "food", docs: ["food"], type: "publicKey" },
            { name: "fuel", docs: ["fuel"], type: "publicKey" },
            { name: "repairKit", docs: ["repair kit"], type: "publicKey" },
          ],
        },
      },
      {
        ...
        type: {
          kind: "struct",
          fields: [
            { name: "crew", docs: ["Number of crew in the ship"], type: "u64" },
            {
              name: "respawnTime",
            ...
            {
              name: "placeholder",
              docs: ["TODO: this is a placeholder stats for use in the future"],
              type: "u32",
            },
            {
              name: "placeholder2",
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "lastUpdate", docs: ["The last update time"], type: "i64" },
          ],
        },
      },
      {
      ...
      },
      {
        ...
        type: { kind: "struct", fields: [{ name: "key", type: "publicKey" }] },
      },
      {
        name: "PlanetType",
          ...
          fields: [
            {
              ...
              type: { defined: "SagePointsCategory" },
            },
            {
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "size", docs: ["`Planet` size"], type: "u64" },
            { name: "maxHp", docs: ["`Planet` max health"], type: "u64" },
            {
              name: "subCoordinates",
              docs: ["`Planet` sub_coordinates"],
              type: { array: ["i64", 2] },
            },
            { name: "planetType", docs: ["`Planet` type"], type: "u8" },
            { name: "position", docs: ["`Planet` position"], type: "u8" },
            {
              name: "keyIndex",
              docs: ["the index of the key in the sector permissions profile"],
              type: "u16",
        ...
        type: {
          ...
          fields: [
            ...
            },
            {
              ...
              type: { defined: "SizeClass" },
            },
            {
              name: "stats",
              docs: ["The stats for the ship"],
              type: { defined: "ShipStatsUnpacked" },
            },
            {
              name: "keyIndex",
              docs: ["the index of the key in the sector permissions profile"],
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "size", docs: ["`Star` size"], type: "u64" },
            {
              name: "subCoordinates",
              docs: ["`Star` sub_coordinates"],
              type: { array: ["i64", 2] },
            },
            { name: "starType", docs: ["`Star` type"], type: "u8" },
            {
              name: "keyIndex",
              docs: ["the index of the key in the sector permissions profile"],
              type: "u16",
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "faction", docs: ["The `Starbase` faction"], type: "u8" },
            {
              name: "keyIndex",
              docs: ["the index of the key in the fleet permissions profile"],
              type: "u16",
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "radius", docs: ["Risk zone radius"], type: "u64" },
          ],
        },
      },
      {
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "flags", docs: ["Connection flags"], type: "u8" },
          ],
        },
      },
      {
        ...
        type: {
          kind: "enum",
          variants: [{ name: "Inner" }, { name: "Mid" }, { name: "Outer" }],
        },
      },
      {
        name: "ShipCounts",
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "xSmall", docs: ["The size of x small ships"], type: "u8" },
            { name: "small", docs: ["The size of small ships"], type: "u8" },
            { name: "medium", docs: ["The size of medium ships"], type: "u8" },
            { name: "large", docs: ["The size of large ships"], type: "u8" },
            {
              name: "capital",
            ...
            { name: "titan", docs: ["The size of titan ships"], type: "u8" },
          ],
        },
      },
      {
          ...
          fields: [
            {
              ...
              type: { defined: "MovementStats" },
            },
            {
              name: "cargoStats",
              docs: ["Cargo stats for the ship"],
              type: { defined: "CargoStats" },
            },
            {
              name: "miscStats",
              docs: ["Miscellaneous stats for the ship"],
              type: { defined: "MiscStats" },
            },
          ],
        },
      },
      ...
      },
      {
        ...
        type: {
          kind: "struct",
          fields: [
            { name: "craftingId", docs: ["crafting id"], type: "u64" },
            {
      ...
      },
      {
        ...
        type: {
          kind: "enum",
          variants: [{ name: "Active" }, { name: "Destroyed" }],
        },
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            },
            {
              name: "info",
              docs: ["The stats for the ship"],
              type: { defined: "StarbaseUpkeepInfoUnpacked" },
            },
          ],
        },
      },
          ...
          fields: [
            {
              ...
              type: { defined: "StarbaseUpkeepInfo" },
            },
            {
              name: "level1",
      ...
      },
      {
        ...
        type: {
          kind: "struct",
          fields: [
            { name: "cargo", docs: ["Cargo settings"], type: "u8" },
            { name: "crafting", docs: ["Crafting settings"], type: "u8" },
            { name: "mints", docs: ["Mints"], type: "u8" },
            { name: "vaults", docs: ["Vaults"], type: "u8" },
            { name: "points", docs: ["Points settings"], type: "u8" },
            {
              name: "riskZones",
              docs: ["Data for risk zones"],
              type: { option: { defined: "RiskZonesDataUnpacked" } },
            },
            {
              name: "keyIndex",
              docs: ["the index of the key in the sector permissions profile"],
          ...
          fields: [
            {
              name: "fleet",
              docs: ["Fleet settings"],
              type: { option: { defined: "FleetInput" } },
            },
            {
              name: "misc",
              docs: ["Miscellaneous settings"],
              type: { option: { defined: "MiscVariablesInput" } },
            },
            {
              name: "keyIndex",
              docs: ["the index of the key in the sector permissions profile"],
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "size", docs: ["`Planet` size"], type: { option: "u64" } },
            {
              name: "maxHp",
              docs: ["`Planet` max_hp"],
              type: { option: "u64" },
        ...
        type: {
          ...
          fields: [
            ...
            },
            {
              ...
              type: {
                option: { vec: { defined: "ProgressionItemInputUnpacked" } },
              },
            },
          ],
        },
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "size", docs: ["`Star` size"], type: { option: "u64" } },
            { name: "starType", docs: ["`Star` type"], type: { option: "u8" } },
            {
              name: "keyIndex",
              docs: ["the index of the key in the sector permissions profile"],
              type: "u16",
        ...
        type: {
          ...
          fields: [
            ...
            },
            {
              ...
              type: { option: { array: [{ array: ["i64", 2] }, 3] } },
            },
            {
              name: "originCoordinates",
              docs: [
      ...
      },
      {
        ...
        type: {
          kind: "enum",
          variants: [{ name: "Ammo" }, { name: "Food" }, { name: "Toolkit" }],
        },
      },
      {
        name: "Vaults",
    ...
    errors: [
      ...
      { code: 6004, name: "InvalidPlanetType", msg: "Invalid Planet Type." },
      { code: 6005, name: "InvalidRingType", msg: "Invalid Ring Type." },
      { code: 6006, name: "InvalidStarType", msg: "Invalid Star Type." },
      {
        code: 6007,
      ...
      { code: 6012, name: "InactiveAccount", msg: "The account is inactive." },
      { code: 6013, name: "InvalidGame", msg: "The game account is invalid." },
      {
        code: 6014,
      ...
      { code: 6019, name: "NumericOverflow", msg: "Numeric overflow" },
      {
        code: 6020,
      ...
      { code: 6029, name: "AuthorityMismatch", msg: "Authority mismatch" },
      { code: 6030, name: "MintMismatch", msg: "Mint mismatch" },
      { code: 6031, name: "TokenMismatch", msg: "Incorrect token address." },
      { code: 6032, name: "OwnerMismatch", msg: "Owner mismatch" },
      { code: 6033, name: "GameMismatch", msg: "Game ID mismatch" },
      { code: 6034, name: "ProfileMismatch", msg: "Profile mismatch" },
      {
        code: 6035,
        name: "SagePlayerProfileMismatch",
        msg: "SagePlayerProfile mismatch",
      },
      { code: 6036, name: "StarbaseMismatch", msg: "Starbase mismatch" },
      { code: 6037, name: "FactionMismatch", msg: "Faction mismatch" },
      { code: 6038, name: "SeqIdMismatch", msg: "Sequence id mismatch" },
      { code: 6039, name: "ShipMismatch", msg: "Ship mismatch" },
      { code: 6040, name: "CargoPodMismatch", msg: "Cargo Pod mismatch" },
      { code: 6041, name: "PlanetMismatch", msg: "Planet mismatch" },
      { code: 6042, name: "MineItemMismatch", msg: "MineItem mismatch" },
      { code: 6043, name: "LocationMismatch", msg: "Location mismatch" },
      {
        code: 6044,
      ...
      { code: 6047, name: "InvalidCrewAmount", msg: "Invalid crew amount" },
      { code: 6048, name: "InvalidState", msg: "Invalid state" },
      ...
      { code: 6052, name: "AddressMismatch", msg: "Address mismatch" },
      {
        code: 6053,
      ...
      { code: 6058, name: "FaultyMovement", msg: "Faulty movement" },
      {
        code: 6059,
      ...
      { code: 6062, name: "MustDisbandFleet", msg: "Must disband fleet" },
      {
        code: 6063,
      ...
      { code: 6065, name: "ShipAlreadyUpdated", msg: "Ship already updated" },
      {
        code: 6066,
      ...
      { code: 6068, name: "InvalidWarpRange", msg: "Warp range exceeded" },
      { code: 6069, name: "InvalidIngredient", msg: "Invalid Ingredient" },
      {
        code: 6070,
        name: "StarbaseUpgradeNotInProgress",
        msg: "Starbase Upgrade Not in progress",
      },
      { code: 6071, name: "FleetNotInQueue", msg: "Fleet Not in queue" },
      {
        code: 6072,
        name: "NeedCleanStarbaseUpgradeQueue",
        msg: "Need to clean Starbase upgrade queue",
      },
      { code: 6073, name: "PlanetNotReachable", msg: "Planet Not Reachable" },
      { code: 6074, name: "RespawnNotPossible", msg: "Respawn Not Possible" },
      {
        code: 6075,
      ...
      { code: 6077, name: "InvalidCargoPod", msg: "The Cargo Pod is invalid" },
      {
        code: 6078,
      ...
      { code: 6080, name: "ActiveAccount", msg: "The Account is Active" },
      {
        code: 6081,
      ...
      { code: 6083, name: "InvalidAmount", msg: "The amount is invalid" },
      { code: 6084, name: "WarpIsOnCooldown", msg: "Warp is on cooldown" },
      { code: 6085, name: "ProgramMismatch", msg: "Program Mismatch" },
      {
        code: 6086,
        name: "MustBeOnlyInstruction",
        msg: "Current Instruction Is Not Only Instruction",
      },
      { code: 6087, name: "InvalidTime", msg: "Invalid Time" },
      { code: 6088, name: "ScanIsOnCooldown", msg: "Scanning is on cooldown" },
      { code: 6089, name: "InvalidFleetSize", msg: "Invalid Fleet Size" },
      { code: 6090, name: "InactiveFeature", msg: "The feature is inactive" },
      { code: 6091, name: "ZeroShipsAdded", msg: "Zero ships added to fleet" },
      { code: 6092, name: "InvalidNoiseSeed", msg: "Invalid Noise Seed" },
      { code: 6093, name: "InvalidType", msg: "Invalid type" },
      { code: 6094, name: "RentedFleet", msg: "Rented Fleet" },
      { code: 6095, name: "GenericInvalid", msg: "Generic invalid data" },
    ],
  };
  const profileIDL = {
    version: "0.7.3",
    ...
    accounts: [
      {
        name: "playerName",
        docs: ["Stores a players name on-chain."],
        type: {
          kind: "struct",
          fields: [
            {
      ...
      },
      {
        ...
        docs: [
          "A players roles for a given profile",
        ...
        type: {
          ...
          fields: [
            ...
            {
              name: "member",
              docs: ["The members profile pubkey"],
              type: "publicKey",
            },
            { name: "bump", docs: ["PDA bump"], type: "u8" },
          ],
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "roleSeqId", docs: ["Roles seq_id"], type: "u64" },
            {
    ...
    types: [
      ...
      {
        name: "RoleMembership",
        docs: ["Represents a members status in a role"],
        type: {
          kind: "struct",
          fields: [
            {
            ...
            { name: "status", docs: ["The members role status"], type: "u8" },
          ],
        },
      },
    ],
    errors: [
      ...
      },
      {
        ...
        msg: "Permissions dont match available",
      },
      {
        code: 6007,
  ...
  const cargoIDL = {
    ...
    instructions: [
      {
        ...
        args: [{ name: "cargoAmount", type: "u64" }],
      },
      {
        name: "closeCargoPod",
        docs: [
      ...
      },
      {
        ...
        accounts: [
          ...
          { name: "mint", isMut: !0, isSigner: !1, docs: ["The Token Mint"] },
          {
            name: "tokenProgram",
            isMut: !1,
            isSigner: !1,
      ...
      },
      {
        ...
        accounts: [
          ...
          { name: "tokenMint", isMut: !0, isSigner: !1, docs: ["Token Mint"] },
          {
            name: "tokenProgram",
            isMut: !1,
            isSigner: !1,
      ...
      },
      {
        ...
        args: [{ name: "podSeeds", type: { array: ["u8", 32] } }],
      },
      {
        name: "initCargoType",
        docs: [
        ...
        accounts: [
          ...
          },
          {
            ...
            isSigner: !1,
            docs: [
              "The cargo permissions [`Profile`].",
              "Is going to act as the authority for the new definition.",
            ],
          },
          {
            name: "funder",
        ...
        args: [{ name: "input", type: { defined: "InitCargoTypeInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "InitCargoTypeFromOldCargoTypeInput" },
          },
        ],
      },
      {
        ...
        accounts: [
          {
            ...
            isSigner: !1,
            docs: [
              "The cargo permissions [`Profile`](Profile).",
              "Is going to act as the authority for the new definition.",
            ],
          },
          {
        ...
        args: [{ name: "input", type: { defined: "InitDefinitionInput" } }],
      },
      {
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "UpdateDefinitionInput" } }],
      },
      {
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "creator", docs: ["TODO: remove this"], type: "publicKey" },
            { name: "bump", docs: ["The bump for this account"], type: "u8" },
            {
              name: "statsCount",
              docs: ["The number of stats in this definition."],
    ...
    errors: [
      ...
      { code: 6001, name: "TooManyStats", msg: "There are too many stats" },
      {
        code: 6002,
      ...
      { code: 6007, name: "InvalidCargoType", msg: "Cargo type is invalid" },
      {
        code: 6008,
      ...
      { code: 6010, name: "NumericOverflow", msg: "Math overflow" },
      { code: 6011, name: "AuthorityMismatch", msg: "Authority mismatch" },
      {
        code: 6012,
        name: "StatsDefinitionMismatch",
        msg: "Stats definition mismatch",
      },
      { code: 6013, name: "MintMismatch", msg: "Mint mismatch" },
      { code: 6014, name: "OwnerMismatch", msg: "Owner mismatch" },
      {
        code: 6015,
        name: "InvalidDelegation",
        msg: "Delegated amount is invalid",
      },
      { code: 6016, name: "FrozenPod", msg: "The pod is frozen" },
      {
        code: 6017,
        name: "UnupdatedCargoPodAccount",
        msg: "Unupdated CargoPod Account",
      },
      { code: 6018, name: "InvalidSeqId", msg: "Invalid seq_id" },
      {
        code: 6019,
  ...
  const craftingIDL = {
    ...
    instructions: [
      {
        ...
        accounts: [
          ...
          { name: "mint", isMut: !1, isSigner: !1, docs: ["The Mint Account"] },
          {
            name: "systemProgram",
            isMut: !1,
            isSigner: !1,
        ...
        args: [{ name: "input", type: { defined: "RecipeIngredients" } }],
      },
      {
      ...
      },
      {
        ...
        accounts: [
          {
            ...
            isSigner: !0,
            docs: [
              "The entity calling this instruction, this is the Crafting Facilitys location",
              "Having the location as the signer is an implicit check that this is the correct location",
            ],
          },
          {
            name: "authority",
        ...
        args: [
          { name: "input", type: { defined: "AddRecipeIngredientInput" } },
        ],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "BurnConsumableIngredientInput" } },
        ],
      },
      {
        name: "cancelCraftingProcess",
      ...
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "ClaimNonConsumableIngredientInput" },
          },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "ClaimRecipeOutputInput" } }],
      },
      {
        name: "closeCraftingProcess",
        accounts: [
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "CreateCraftingProcessInput" } },
        ],
      },
      {
      ...
      },
      {
        ...
        args: [{ name: "namespace", type: { array: ["u8", 32] } }],
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "LegitimizeRecipeIngredientInput" },
          },
        ],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "RegisterCraftableItemInput" } },
        ],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "RegisterCraftingFacilityInput" } },
        ],
      },
      {
        name: "registerRecipe",
        ...
        args: [{ name: "input", type: { defined: "RegisterRecipeInput" } }],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "RegisterRecipeCategoryInput" } },
        ],
      },
      {
        ...
        args: [{ name: "input", type: { defined: "RemoveRecipeIngredients" } }],
      },
      {
        ...
        args: [
          {
            name: "input",
            type: { defined: "RemoveCraftingFacilityRecipeCategoryInput" },
          },
        ],
      },
      {
      ...
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "RemoveRecipeIngredientInput" } },
        ],
      },
      {
        name: "startCraftingProcess",
        ...
        args: [
          { name: "input", type: { defined: "StartCraftingProcessInput" } },
        ],
      },
      {
        name: "stopCraftingProcess",
        ...
        args: [
          { name: "input", type: { defined: "StopCraftingProcessInput" } },
        ],
      },
      {
        ...
        args: [
          { name: "input", type: { defined: "UpdateCraftingFacilityInput" } },
        ],
      },
      {
        name: "updateDomain",
      ...
      },
      {
        ...
        args: [{ name: "input", type: { defined: "UpdateRecipeInput" } }],
      },
      {
    ...
    accounts: [
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "domain", docs: ["the domain"], type: "publicKey" },
            {
              name: "mint",
      ...
      {
        name: "craftingFacility",
        docs: ["Represents a crafting facility account"],
        type: {
          kind: "struct",
          fields: [
            {
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "craftingId", docs: ["the crafting id"], type: "u64" },
            {
              name: "authority",
              docs: ["the owner/authority of this crafting process"],
              type: "publicKey",
            },
            { name: "recipe", docs: ["the recipe"], type: "publicKey" },
            {
              name: "craftingFacility",
            ...
            { name: "startTime", docs: ["the start timestamp"], type: "i64" },
            { name: "endTime", docs: ["the end timestamp"], type: "i64" },
            {
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            { name: "status", docs: ["The status of the recipe"], type: "u8" },
            {
              name: "feeAmount",
            ...
            {
              name: "usageCount",
              docs: ["The number of times that this recipe has been used"],
              type: "u64",
            },
            {
              name: "usageLimit",
    ...
    types: [
      ...
      },
      {
        ...
        type: {
          ...
          fields: [
            ...
            {
              name: "recipeCategoryIndex",
              docs: ["the index of the recipes category"],
              type: "u16",
            },
            {
              name: "quantity",
      ...
      },
      {
        ...
        type: { kind: "enum", variants: [{ name: "Starbase" }] },
      },
      {
        name: "OptionalNonSystemPubkey",
        docs: [
      ...
      },
      {
        ...
        type: {
          kind: "enum",
          variants: [{ name: "Initialized" }, { name: "Started" }],
        },
      },
      {
      ...
      {
        name: "StartCraftingProcessInput",
        docs: ["Inputs for `StartCraftingProcess`"],
        type: {
          kind: "struct",
          fields: [
            {
              name: "recipeDurationOverride",
              docs: ["recipe duration override"],
              type: { option: "u64" },
            },
            {
              name: "localTime",
      ...
      {
        name: "StopCraftingProcessInput",
        docs: ["Inputs for `StopCraftingProcess`"],
        type: {
          kind: "struct",
          fields: [
            {
      ...
      },
      {
        ...
        docs: [
          ...
          "We are wrapping this around Pubkey so that we can use Anchors coder to decode it",
        ],
        type: {
          kind: "struct",
          fields: [
    ...
    errors: [
      ...
      { code: 6002, name: "NumericOverflow", msg: "Numeric overflow" },
      {
        code: 6003,
        name: "MintAuthorityIsNone",
        msg: "The mint authority should exist.",
      ...
      { code: 6008, name: "IncorrectDomain", msg: "The domain is incorrect." },
      { code: 6009, name: "IncorrectRecipe", msg: "The recipe is incorrect." },
      {
        code: 6010,
      ...
      {
        code: 6022,
        name: "CraftingProcessAlreadyCompleted",
        msg: "The crafting process has already completed.",
      },
      {
        code: 6023,
      ...
      { code: 6033, name: "InvalidDuration", msg: "Duration is invalid." },
      {
        code: 6034,
      ...
      { code: 6037, name: "InvalidRecipeStatus", msg: "Invalid Recipe Status" },
      {
        code: 6038,
        name: "EfficiencyIsZero",
        msg: "Crafting Facility Efficiency Is Zero",
      },
      { code: 6039, name: "AddressMismatch", msg: "Address Mismatch" },
      { code: 6040, name: "RecipeLimitExceeded", msg: "Recipe Limit Exceeded" },
    ],
  };
  ...
  let sageProgram = new BrowserAnchor.anchor.Program(
    sageIDL,
    window.blockchainManager.sageProgramPK,
    anchorProvider
  );
  let [sageGameAcct] = await sageProgram.account.game.all();
  let [sageSDUTrackerAcct] =
    await sageProgram.account.surveyDataUnitTracker.all();
  let profileProgram = new BrowserAnchor.anchor.Program(
    profileIDL,
    window.blockchainManager.profileProgramPK,
    anchorProvider
  );
  let pointsProgram = new BrowserAnchor.anchor.Program(
    pointsIDL,
    window.blockchainManager.pointsProgramId,
    anchorProvider
  );
  let craftingProgram = new BrowserAnchor.anchor.Program(
    craftingIDL,
    window.blockchainManager.craftingProgramPK,
    anchorProvider
  );
  let cargoProgram = new BrowserAnchor.anchor.Program(
    cargoIDL,
    window.blockchainManager.cargoProgramPK,
    anchorProvider
  );
  let [cargoStatsDefinitionAcct] =
    await cargoProgram.account.cargoStatsDefinition.all();
  let cargoStatsDefSeqId = cargoStatsDefinitionAcct.account.seqId;
  let seqBN = new BrowserAnchor.anchor.BN(cargoStatsDefSeqId);
  let seqArr = seqBN
    .toTwos(64)
    .toArrayLike(BrowserBuffer.Buffer.Buffer, "be", 2);
  ...
  await window.game.getResourceTokens();

  let sduItem = cargoItems.find((item) => item.name === "Survey Data Unit");
  let fuelItem = cargoItems.find(
    (item) => item.token === sageGameAcct.account.mints.fuel.toString()
  );
  let foodItem = cargoItems.find(
    (item) => item.token === sageGameAcct.account.mints.food.toString()
  );
  let toolItem = cargoItems.find(
    (item) => item.token === sageGameAcct.account.mints.repairKit.toString()
  );
  let ammoItem = cargoItems.find(
    (item) => item.token === sageGameAcct.account.mints.ammo.toString()
  );

  let [progressionConfigAcct] =
    await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
      [
  ...
  await window.game.initUser();
  let autoSpanRef = document.querySelector("#autoScanBtn > span");
  autoSpanRef ? (autoSpanRef.innerHTML = "Start") : null;
  window.logger.cLog(0, "init complete");
  window.logger.cLog(0, "Fleets: ", userFleets);
})();
