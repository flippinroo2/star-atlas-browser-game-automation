...
(async function () {
  ...
  class Assistant {
    ...
    async saveSettingsInput() {
      ...
      globalSettings = {
        priorityFee: parseIntDefault(
          document.querySelector("#priorityFee").value,
          1
        ),
        lowPriorityFeeMultiplier: parseIntDefault(
        ...
        savedProfile: saveProfile
          ? userProfileAcct && userProfileKeyIdx
            ...
            : []
          : [],
        confirmationCheckingDelay: parseIntDefault(
        ...
        scanBlockLength: parseIntDefault(
        ...
        get(target, key, receiver) {
          ...
          if (typeof origMethod === "function") {
            return async function (...args) {
              solanaReadCount++;
              debugger;
              return await this.doProxyStuff(target, origMethod, args, readRPCs, "READ");
            };
          }
        },
      };
  ...
  /*
    ...
    */
  const sageProgramPK = new solanaWeb3.PublicKey(
    "SAGE2HAwep459SNq61LHvjxPk4pLPEJLoMETef7f7EE"
  );
  const profileProgramPK = new solanaWeb3.PublicKey(
    "pprofELXjL5Kck7Jn5hCpwAL82DpTkSYBENzahVtbc9"
  );
  const cargoProgramPK = new solanaWeb3.PublicKey(
    "Cargo2VNTPPTi9c1vq1Jw5d3BWUNr18MjRtSupAghKEk"
  );
  const profileFactionProgramPK = new solanaWeb3.PublicKey(
    "pFACSRuobDmvfMKq1bAzwj27t6d2GJhSCHb1VcfnRmq"
  );
  const craftingProgramPK = new solanaWeb3.PublicKey(
    "CRAFT2RPXPJWCEix4WpJST3E7NLf79GTqZUL75wngXo5"
  );
  const pointsProgramId = new solanaWeb3.PublicKey(
    "Point2iBvz7j5TMVef8nEgpmz4pDr7tU7v3RjAfkQbM"
  );
  const dataRunningXpCategory = new solanaWeb3.PublicKey(
    "DataJpxFgHhzwu4zYJeHCnAv21YqWtanEBphNxXBHdEY"
  );
  const councilRankXpCategory = new solanaWeb3.PublicKey(
    "XPneyd1Wvoay3aAa24QiKyPjs8SUbZnGg5xvpKvTgN9"
  ); //??
  const pilotingXpCategory = new solanaWeb3.PublicKey(
    "PiLotBQoUBUvKxMrrQbuR3qDhqgwLJctWsXj3uR7fGs"
  );
  const miningXpCategory = new solanaWeb3.PublicKey(
    "MineMBxARiRdMh7s1wdStSK4Ns3YfnLjBfvF5ZCnzuw"
  );
  const craftingXpCategory = new solanaWeb3.PublicKey(
  ...
  const readIdx =
    customReadRPCs.length > 0 ? 0 : Math.floor(Math.random() * saRPCs.length);
  const writeIdx =
    customWriteRPCs.length > 0 ? 0 : Math.floor(Math.random() * saRPCs.length);
  const rawSolanaReadConnection = new solanaWeb3.Connection(
    readRPCs[readIdx],
    "confirmed"
  );
  const solanaReadConnection = new Proxy(
    rawSolanaReadConnection,
    readConnectionProxy
  );
  const rawSolanaWriteConnection = new solanaWeb3.Connection(
  ...
  const solanaWriteConnection = new Proxy(
    rawSolanaWriteConnection,
    writeConnectionProxy
  );
  ...
  async function getSolanaClockTime() {
    let solanaClock = await solanaReadConnection.getAccountInfo(
  ...
  async function getCargoTypeSize(cargoType) {
    let cargoTypeAcct = await solanaReadConnection.getAccountInfo(
      cargoType.publicKey
    );
  ...
  function calculateMiningDuration(
    cargoCapacity,
    miningRate,
    resourceHardness,
    systemRichness
    ...
    try {
      ...
      for (let i = 0; i < gridSize; i++) {
        ...
        for (let j = 0; j < gridSize; j++) {
          ...
          let fleetAccts = await solanaReadConnection.getProgramAccounts(
            sageProgramPK,
            {
              filters: [
  ...
  async function sendAndConfirmTx(
    ...
    let { blockHeight: curBlockHeight } =
      await solanaReadConnection.getEpochInfo({ commitment: "confirmed" });
    ...
    while (curBlockHeight - interimBlockHeight < 10) {
      const signatureStatus = await solanaReadConnection.getSignatureStatus(
        txHash
      );
      if (
      ...
      let epochInfo = await solanaReadConnection.getEpochInfo({
    ...
    return new Promise(async (resolve) => {
      ...
      while (!confirmed) {
        ...
        const priorityFee = globalSettings.priorityFee
          ? Math.max(
              1,
              Math.ceil(priorityFeeMultiplier * globalSettings.priorityFee * 5)
            )
        ...
        let latestBH = await solanaReadConnection.getLatestBlockhash(
          "confirmed"
        );
        ...
        let response = await sendAndConfirmTx(
          txSerialized,
        ...
        let txResult = txHash
          ? await solanaReadConnection.getTransaction(txHash, {
              commitment: "confirmed",
              preflightCommitment: "confirmed",
  ...
  async function execStartupUndock(i, assignment) {
    ...
    if (assignment == "Transport" || assignment == "Mine") {
      const fleetAcctInfo = await solanaReadConnection.getAccountInfo(
        fleet.publicKey
      );
  ...
  async function execCargoFromFleetToStarbase(
    fleet,
    fleetCargoPod,
    tokenMint,
    dockCoords,
  ...
  async function execCargoFromStarbaseToFleet(
    fleet,
    cargoPodTo,
    tokenTo,
    tokenMint,
        ...
        if (cargoHold.account && cargoHold.account.openTokenAccounts > 0) {
          ...
          if (cargoHoldFound) {
            ...
            let cargoHoldFound = cargoHoldTokens.value.find(
              (item) =>
                item.account.data.parsed.info.mint === tokenMint &&
                item.account.data.parsed.info.tokenAmount.uiAmount >= mostFound
            );
            if (cargoHoldFound) {
              starbasePlayerCargoHold = cargoHold;
              mostFound =
  ...
  async function execStopMining(
    ...
    return new Promise(async (resolve) => {
      ...
      (await solanaReadConnection.getAccountInfo(fleetAmmoAcct)) ||
        (await createPDA(
          fleetAmmoAcct,
          fleet.ammoBank,
  ...
  async function execLoadFleetAmmo(fleet, starbaseCoords, amount) {
    ...
    const parsedTokenAccounts =
      await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.ammoBank, {
        programId: tokenProgramPK,
      });
  ...
  async function addAssistInput(fleet) {
    ...
    scanMin.value =
      fleetParsedData && fleetParsedData.scanMin ? fleetParsedData.scanMin : "";
...
*/
    ...
    let transportResource1Token =
      ...
      fleetParsedData.transportResource1 !== ""
        ? cargoItems.find((r) => r.token == fleetParsedData.transportResource1)
        : "";
    ...
    let transportResource2Token =
      ...
      fleetParsedData.transportResource2 !== ""
        ? cargoItems.find((r) => r.token == fleetParsedData.transportResource2)
        : "";
    ...
    let transportResource3Token =
      ...
      fleetParsedData.transportResource3 !== ""
        ? cargoItems.find((r) => r.token == fleetParsedData.transportResource3)
        : "";
    ...
    let transportResource4Token =
      ...
      fleetParsedData.transportResource4 !== ""
        ? cargoItems.find((r) => r.token == fleetParsedData.transportResource4)
        : "";
  ...
  function updateAssistStatus(fleet) {
    let targetRow = document.querySelectorAll(
      '#assistStatus .assist-fleet-row[pk="' + fleet.publicKey.toString() + '"]'
    );
  ...
  async function handleUndockAll() {
    for (let i = 0, n = userFleets.length; i < n; i++) {
      let fleetAcctInfo = await solanaReadConnection.getAccountInfo(
        userFleets[i].publicKey
      );
      let [fleetState, extra] = getFleetState(fleetAcctInfo);
      if (fleetState === "StarbaseLoadingBay") {
    ...
    let cargoCnt = fleetCurrentCargo.value.reduce(
      (n, { account }) =>
        ...
        account.data.parsed.info.tokenAmount.uiAmount *
          cargoItems.find((r) => r.token == account.data.parsed.info.mint).size,
      0
    );
  ...
  async function handleResupply(i, fleetCoords) {
    ...
    async function loadFood() {
      ...
      //Update food count
      const foodCargo = await solanaReadConnection.getTokenAccountBalance(
        userFleets[i].foodToken
      );
      ...
      if (userFleets[i].foodCnt < userFleets[i].scanCost) {
        ...
        updateFleetState(
          userFleets[i],
          `ERROR: No food ⌛ ${TimeToStr(new Date(Date.now() + errorWaitTime))}`
        );
    ...
    async function loadFuel() {
      ...
      let currentFuelCnt = fleetCurrentFuel.value.find(
        (item) => item.pubkey.toString() === userFleets[i].fuelToken.toString()
      );
      ...
      if (userFleets[i].fuelCnt < userFleets[i].fuelCapacity * errorFuelRatio) {
        ...
        updateFleetState(
          userFleets[i],
          `ERROR: No Fuel ⌛ ${TimeToStr(new Date(Date.now() + errorWaitTime))}`
        );
        ...
        if (fleetCoords[0] == starbaseX && fleetCoords[1] == starbaseY) {
          ...
          if (currentFuelCnt < fuelNeeded) {
            ...
            if (fuelResp && fuelResp.name == "NotEnoughResource") {
              cLog(
                1,
                `${FleetTimeStamp(userFleets[i].label)} ERROR: Not enough fuel`
              );
          ...
          if (currentAmmoCnt < ammoForDuration) {
            ...
            if (ammoResp && ammoResp.name == "NotEnoughResource") {
              cLog(
                1,
                `${FleetTimeStamp(userFleets[i].label)} ERROR: Not enough ammo`
              );
          ...
          if (currentFoodCnt < foodForDuration) {
            ...
            if (foodResp && foodResp.name == "NotEnoughResource") {
              cLog(
                1,
                `${FleetTimeStamp(userFleets[i].label)} ERROR: Not enough food`
              );
  ...
  async function getFleetFuelData(fleet, currentPos, targetPos) {
    ...
    const fleetCurrentFuelTank =
      await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.fuelTank, {
        programId: tokenProgramPK,
      });
  ...
  async function fuelFleet(fleet, dockCoords, account, amount) {
    ...
    let fuelCargoTypeAcct = cargoTypes.find(
      (item) => item.account.mint.toString() == sageGameAcct.account.mints.fuel
    );
  ...
  async function handleTransportRefueling(
    fleet,
    starbaseCoord,
    currentPos,
    targetPos,
    ...
    if (amountToDropOff > 0) {
      ...
      if (fuelToUnload > 0) {
        cLog(
          1,
          `${FleetTimeStamp(fleet.label)} Unloading extra fuel: ${fuelToUnload}`
        );
        await execCargoFromFleetToStarbase(
          fleet,
          fleet.fuelTank,
  ...
  async function handleTransportUnloading(
    fleet,
    starbaseCoord,
    transportManifest
  ) {
    ...
    if (ammoEntry) {
      ...
      if (ammoToUnload > 0) {
        cLog(
          1,
          `${FleetTimeStamp(fleet.label)} Unloading Ammobanks: ${ammoToUnload}`
        );
        await execCargoFromFleetToStarbase(
          fleet,
          fleet.ammoBank,
      ...
      if (entry.res && entry.amt > 0) {
        ...
        if (resMax > 0) {
          cLog(
            ...
            `${FleetTimeStamp(
              userFleets[i].label
            )} Attempting to load ${resMax} ${entry.res} from ${starbaseCoords}`
          );
  ...
  function initUser() {
    return new Promise(async (resolve) => {
      ...
      if (
        ...
        let userProfiles = await solanaReadConnection.getProgramAccounts(
          profileProgramPK
        );
        let foundProf = [];
        ...
        for (let userProf of userProfiles) {
          ...
          while (userProfData.length >= 80) {
            ...
            if (profDecoded.key.toString() === userPublicKey.toString()) {
              let [playerNameAcct] =
                await solanaReadConnection.getProgramAccounts(
                  profileProgramPK,
                  {
                    filters: [
                      {
